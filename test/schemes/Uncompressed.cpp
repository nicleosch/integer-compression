...
// decompression.
TEST(UncompressedTest, ColumnDecompressionInvariant32bit) {
  ...
  ColumnCompressor<INTEGER, kBlockSize> compressor(
      column, CompressionSchemeType::kUncompressed);
...
// Verifies that the data remains unchanged after columnar compression and
// decompression.
TEST(UncompressedTest, ColumnDecompressionInvariant64bit) {
  constexpr uint16_t kBlockSize = 256;
