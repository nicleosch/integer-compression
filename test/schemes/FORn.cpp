...
// decompression.
TEST(FORnTest, ColumnDecompressionInvariant) {
  constexpr uint16_t kBlockSize = 256;

  auto path = "../data/tpch/sf1/partsupp.tbl";
  auto column = storage::Column::fromFile(path, 0, '|');

  ColumnCompressor<kBlockSize> compressor(column, CompressionSchemeType::kFORn);
...
// decompression.
TEST(FORnTest, BlockDecompressionInvariant) {
  constexpr uint16_t kBlockSize = 256;
