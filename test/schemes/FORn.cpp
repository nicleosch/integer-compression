...
TEST(FORnTest, ColumnDecompressionInvariant32bit) {
  ...
  ColumnCompressor<INTEGER, kBlockSize> compressor(
      column, CompressionSchemeType::kFORn);
...
// decompression for 32 bit integers.
TEST(FORnTest, BlockDecompressionInvariant32bit) {
  constexpr uint16_t kBlockSize = 256;
...
TEST(FORnTest, ColumnDecompressionInvariant64bit) {
  constexpr uint16_t kBlockSize = 256;
...
// Verifies that the data remains unchanged after block-wise compression and
// decompression for 64 bit integers.
TEST(FORnTest, BlockDecompressionInvariant64bit) {
  constexpr uint16_t kBlockSize = 256;
...
//---------------------------------------------------------------------------
// Verifies that the data remains unchanged after compression using FORn
// and a second round of compression using LZ4 on the header.
TEST(FORnTest, Phase2HeaderCompressionInvariant) {
  constexpr uint16_t kBlockSize = 256;
...
TEST(FORnTest, Phase2PayloadCompressionInvariant) {
  constexpr uint16_t kBlockSize = 256;
...
TEST(FORnTest, Phase2CompressionInvariant) {
  constexpr uint16_t kBlockSize = 256;
