...
u32 FORn::compress(
  ...
  return compressImpl(src, dest, stats, total_size, block_size, BitPacking::packFixed);
}
//---------------------------------------------------------------------------
...
u32 FORn::compressImpl(
  ...
  for(u32 block_i = 0; block_i < block_count; ++block_i) {
    ...
    u8 pack_size = FOR::compressImpl(src, data_ptr + offset, &stats[block_i], size, bitpack_func);
