...
u32 FOR::compress(const INTEGER *src, u8 *dest, const Statistics *stats,
  ...
  layout.pack_size =
      compressImpl(src, layout.data, stats, total_size, BitPacking::packFixed);
...
void FOR::decompress(INTEGER *dest, const u8 *src, const u32 total_size,
  ...
  decompressImpl(dest, layout.data, total_size, layout.reference,
                 layout.pack_size);
}
//---------------------------------------------------------------------------
CompressionSchemeType FOR::getType() { return CompressionSchemeType::kFOR; }
//---------------------------------------------------------------------------
u8 FOR::compressImpl(const INTEGER *src, u8 *dest, const Statistics *stats,
  ...
  // Compress
  u8 pack_size = bitpack_func(normalized.data(), dest, total_size,
                              stats->max - stats->min);

  return pack_size;
}
//---------------------------------------------------------------------------
void FOR::decompressImpl(INTEGER *dest, const u8 *src, const u32 total_size,
