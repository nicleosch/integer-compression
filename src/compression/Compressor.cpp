...
//---------------------------------------------------------------------------
static void computeStats(storage::Column& col, vector<compression::Statistics>& stats, u32 block_size) {
...
//---------------------------------------------------------------------------
u32 compressor::compressFORn(storage::Column col, std::unique_ptr<u8[]>& dest, u32 block_size) {
  // allocate space
  dest = std::make_unique<u8[]>(col.size() * sizeof(INTEGER));
...
//---------------------------------------------------------------------------
u32 compressor::compressAdaptiveFORn(storage::Column col, std::unique_ptr<u8[]>& dest, u32 block_size) {
  // allocate space
  dest = std::make_unique<u8[]>(col.size() * sizeof(INTEGER));
  ...
  return cAdaptiveForN.compress(col.data(), dest.get(), stats.data(), col.size(), block_size);
};
//---------------------------------------------------------------------------
}  // compression