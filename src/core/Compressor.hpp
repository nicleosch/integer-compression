...
/// delta compression.
/// @return The size of the compressed data in bytes.
u32 compressDelta(storage::Column col, std::unique_ptr<u8[]> &dest);
//---------------------------------------------------------------------------
/// @brief Compresses given column to provided destination using
/// run-length encoding.
/// @return The size of the compressed data in bytes.
u32 compressRLE(storage::Column col, std::unique_ptr<u8[]> &dest);
//---------------------------------------------------------------------------
/// @brief Compresses given column to provided destination using
...
u32 compressBitPacking(storage::Column col, std::unique_ptr<u8[]> &dest) {
  // allocate space
  ...
  // compress
  BitPacking cBP;
  return cBP.compress<kBlockSize>(col.data(), col.size(), dest.get(),
                                  stats.data());
}
//---------------------------------------------------------------------------
/// @brief Compresses given column to provided destination using
