# Generates code for AVXBitPacking512.cpp
...
print("""typedef void (*avx512packblockfnc)(const u64 * pin, __m512i * compressed);""")
print("""typedef void (*avx512unpackblockfnc)(const __m512i * compressed, u64 * pout);""")
...
print("")
print("static void avx512packblock0(const u64 * pin, __m512i * compressed) {")
...
for bit in range(1,65):
    ...
    print("static void avx512packblock{0}(const u64 * pin, __m512i * compressed) {{".format(bit))
    ...
    for j in range(int(howmany(bit)/8)):
      ...
      if( firstword == secondword):
          if(firstshift == 0):
            ...
            print("  w{0} = _mm512_or_si512(w{0},_mm512_slli_epi64(_mm512_loadu_si512 (in + {1}) , {2}));".format(firstword%2,j,firstshift))
      else:
          print("  tmp = _mm512_loadu_si512 (in + {0});".format(j))
          print("  w{0} = _mm512_or_si512(w{0},_mm512_slli_epi64(tmp , {2}));".format(firstword%2,j,firstshift))
          secondshift = 64-firstshift
          print("  w{0} = _mm512_srli_epi64(tmp,{2});".format(secondword%2,j,secondshift))
...
print("")
print("static void avx512packblockmask0(const u64 * pin, __m512i * compressed) {")
print("  (void)compressed;")
print("  (void) pin; /* we consumed {0} 64-bit integer{1} */ ".format(howmany(0),plurial(howmany(0))))
print("}")
print("")
...
for bit in range(1,65):
    print("")
    print("/* we are going to pack {0} {1}-bit values, touching {2} 512-bit words, using {3} bytes */ ".format(howmany(bit),bit,howmanywords(bit),howmanybytes(bit)))
    print("static void avx512packblockmask{0}(const u64 * pin, __m512i * compressed) {{".format(bit))
    print("  /* we are going to touch  {0} 512-bit word{1} */ ".format(howmanywords(bit),plurial(howmanywords(bit))))
    if(howmanywords(bit) == 1):
      print("  __m512i w0;")
    else:
      print("  __m512i w0, w1;")
    print("  const __m512i * in = (const __m512i *) pin;")
    if(bit < 64): print("  const __m512i mask = _mm512_set1_epi64({0});".format((1<<bit)-1));
    def maskfnc(x):
        if(bit == 64): return x
    ...
    for j in range(int(howmany(bit)/8)):
      ...
      if( firstword == secondword):
          if(firstshift == 0):
            ...
            print("  w{0} = _mm512_or_si512(w{0},_mm512_slli_epi64({1} , {2}));".format(firstword%2,loadstr,firstshift))
      else:
...
print("static void avx512unpackblock0(const __m512i * compressed, u64 * pout) {");
print("  (void) compressed;")
...
for bit in range(1,65):
    ...
    print("static void avx512unpackblock{0}(const __m512i * compressed, u64 * pout) {{".format(bit));
    ...
    for j in range(int(howmany(bit)/8)):
      ...
      if(secondword > oldword):
        ...
        oldword = secondword
      firstshift = (j*bit) % 64
      firstshiftstr = "_mm512_srli_epi64( w{0} , "+str(firstshift)+") "
      if(firstshift == 0):
      ...
      if( firstword == secondword):
          ...
          secondshift = (64-firstshift)
          wsecond = "_mm512_slli_epi64( w{0} , {1} ) ".format((firstword+1)%2,secondshift)
