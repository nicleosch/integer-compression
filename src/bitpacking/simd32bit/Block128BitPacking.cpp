...
#include <simdcomp.h>
//---------------------------------------------------------------------------
#include "Block128BitPacking.hpp"
//---------------------------------------------------------------------------
...
static void filtereq0(const __m128i *in, u32 *matches, const INTEGER comp) {
  if (comp == 0)
    memset(matches, 1, 128 * sizeof(*matches));
  else
    memset(matches, 0, 128 * sizeof(*matches));
}

/* we packed 128 1-bit values, touching 2 128-bit words, using 32 bytes */
static void filtereq1(const __m128i *in, u32 *matches, const INTEGER comp) {
  ...
  _mm_storeu_si128(
      out + 19,
      _mm_cmpeq_epi32(_mm_and_si128(mask, _mm_srli_epi32(w0, 19)), broadcomp));
  _mm_storeu_si128(
      out + 20,
  ...
  _mm_storeu_si128(
      out + 23,
      _mm_cmpeq_epi32(_mm_and_si128(mask, _mm_srli_epi32(w0, 23)), broadcomp));
  _mm_storeu_si128(
      out + 24,
  ...
  _mm_storeu_si128(
      out + 29,
      _mm_cmpeq_epi32(_mm_and_si128(mask, _mm_srli_epi32(w0, 29)), broadcomp));
  _mm_storeu_si128(
      out + 30,
      _mm_cmpeq_epi32(_mm_and_si128(mask, _mm_srli_epi32(w0, 30)), broadcomp));
  _mm_storeu_si128(out + 31,
                   _mm_cmpeq_epi32(_mm_srli_epi32(w0, 31), broadcomp));
}

/* we packed 128 2-bit values, touching 3 128-bit words, using 48 bytes */
static void filtereq2(const __m128i *in, u32 *matches, const INTEGER comp) {
  ...
  _mm_storeu_si128(
      out + 29,
      _mm_cmpeq_epi32(_mm_and_si128(mask, _mm_srli_epi32(w1, 26)), broadcomp));
  _mm_storeu_si128(
      out + 30,
      _mm_cmpeq_epi32(_mm_and_si128(mask, _mm_srli_epi32(w1, 28)), broadcomp));
  _mm_storeu_si128(out + 31,
                   _mm_cmpeq_epi32(_mm_srli_epi32(w1, 30), broadcomp));
}

/* we packed 128 3-bit values, touching 4 128-bit words, using 64 bytes */
static void filtereq3(const __m128i *in, u32 *matches, const INTEGER comp) {
  ...
  _mm_storeu_si128(
      out + 17,
      _mm_cmpeq_epi32(_mm_and_si128(mask, _mm_srli_epi32(w1, 19)), broadcomp));
  _mm_storeu_si128(
      out + 18,
      _mm_cmpeq_epi32(_mm_and_si128(mask, _mm_srli_epi32(w1, 22)), broadcomp));
  _mm_storeu_si128(
      out + 19,
      _mm_cmpeq_epi32(_mm_and_si128(mask, _mm_srli_epi32(w1, 25)), broadcomp));
  _mm_storeu_si128(
      out + 20,
      _mm_cmpeq_epi32(_mm_and_si128(mask, _mm_srli_epi32(w1, 28)), broadcomp));
  ...
  _mm_storeu_si128(
      ...
      _mm_cmpeq_epi32(_mm_and_si128(mask, _mm_srli_epi32(w0, 26)), broadcomp));
  _mm_storeu_si128(out + 31,
                   _mm_cmpeq_epi32(_mm_srli_epi32(w0, 29), broadcomp));
}

/* we packed 128 4-bit values, touching 5 128-bit words, using 80 bytes */
static void filtereq4(const __m128i *in, u32 *matches, const INTEGER comp) {
  ...
  _mm_storeu_si128(out + 23,
                   _mm_cmpeq_epi32(_mm_srli_epi32(w0, 28), broadcomp));
...
/* we packed 128 5-bit values, touching 6 128-bit words, using 96 bytes */
static void filtereq5(const __m128i *in, u32 *matches, const INTEGER comp) {
  ...
  _mm_storeu_si128(
      ...
      _mm_cmpeq_epi32(_mm_and_si128(mask, _mm_srli_epi32(w0, 22)), broadcomp));
  _mm_storeu_si128(out + 31,
                   _mm_cmpeq_epi32(_mm_srli_epi32(w0, 27), broadcomp));
}

/* we packed 128 6-bit values, touching 7 128-bit words, using 112 bytes */
static void filtereq6(const __m128i *in, u32 *matches, const INTEGER comp) {
  ...
  _mm_storeu_si128(
      ...
      _mm_cmpeq_epi32(_mm_and_si128(mask, _mm_srli_epi32(w1, 20)), broadcomp));
  _mm_storeu_si128(out + 31,
                   _mm_cmpeq_epi32(_mm_srli_epi32(w1, 26), broadcomp));
}

/* we packed 128 7-bit values, touching 8 128-bit words, using 128 bytes */
static void filtereq7(const __m128i *in, u32 *matches, const INTEGER comp) {
  ...
  _mm_storeu_si128(
      out + 25,
      _mm_cmpeq_epi32(_mm_and_si128(mask, _mm_srli_epi32(w1, 15)), broadcomp));
  _mm_storeu_si128(
      out + 26,
      _mm_cmpeq_epi32(_mm_and_si128(mask, _mm_srli_epi32(w1, 22)), broadcomp));
  ...
  _mm_storeu_si128(
      ...
      _mm_cmpeq_epi32(_mm_and_si128(mask, _mm_srli_epi32(w0, 18)), broadcomp));
  _mm_storeu_si128(out + 31,
                   _mm_cmpeq_epi32(_mm_srli_epi32(w0, 25), broadcomp));
}

/* we packed 128 8-bit values, touching 9 128-bit words, using 144 bytes */
...
/* we packed 128 9-bit values, touching 10 128-bit words, using 160 bytes */
static void filtereq9(const __m128i *in, u32 *matches, const INTEGER comp) {
  ...
  _mm_storeu_si128(
      ...
      _mm_cmpeq_epi32(_mm_and_si128(mask, _mm_srli_epi32(w0, 14)), broadcomp));
  _mm_storeu_si128(out + 31,
                   _mm_cmpeq_epi32(_mm_srli_epi32(w0, 23), broadcomp));
}

/* we packed 128 10-bit values, touching 11 128-bit words, using 176 bytes */
static void filtereq10(const __m128i *in, u32 *matches, const INTEGER comp) {
  ...
  _mm_storeu_si128(
      ...
      _mm_cmpeq_epi32(_mm_and_si128(mask, _mm_srli_epi32(w1, 12)), broadcomp));
  _mm_storeu_si128(out + 31,
                   _mm_cmpeq_epi32(_mm_srli_epi32(w1, 22), broadcomp));
}

/* we packed 128 11-bit values, touching 12 128-bit words, using 192 bytes */
static void filtereq11(const __m128i *in, u32 *matches, const INTEGER comp) {
  ...
  _mm_storeu_si128(
      ...
      _mm_cmpeq_epi32(_mm_and_si128(mask, _mm_srli_epi32(w0, 10)), broadcomp));
  _mm_storeu_si128(out + 31,
                   _mm_cmpeq_epi32(_mm_srli_epi32(w0, 21), broadcomp));
}

/* we packed 128 12-bit values, touching 13 128-bit words, using 208 bytes */
static void filtereq12(const __m128i *in, u32 *matches, const INTEGER comp) {
  ...
  _mm_storeu_si128(
      ...
      _mm_cmpeq_epi32(_mm_and_si128(mask, _mm_srli_epi32(w0, 8)), broadcomp));
  _mm_storeu_si128(out + 23,
                   _mm_cmpeq_epi32(_mm_srli_epi32(w0, 20), broadcomp));
  w1 = _mm_lddqu_si128(in + 9);
  _mm_storeu_si128(out + 24,
                   _mm_cmpeq_epi32(_mm_and_si128(mask, w1), broadcomp));
  _mm_storeu_si128(
...
/* we packed 128 13-bit values, touching 14 128-bit words, using 224 bytes */
static void filtereq13(const __m128i *in, u32 *matches, const INTEGER comp) {
  ...
  _mm_storeu_si128(
      ...
      _mm_cmpeq_epi32(_mm_and_si128(mask, _mm_srli_epi32(w0, 6)), broadcomp));
  _mm_storeu_si128(out + 31,
                   _mm_cmpeq_epi32(_mm_srli_epi32(w0, 19), broadcomp));
}

/* we packed 128 14-bit values, touching 15 128-bit words, using 240 bytes */
static void filtereq14(const __m128i *in, u32 *matches, const INTEGER comp) {
  ...
  _mm_storeu_si128(
      ...
      _mm_cmpeq_epi32(_mm_and_si128(mask, _mm_srli_epi32(w1, 4)), broadcomp));
  _mm_storeu_si128(out + 31,
                   _mm_cmpeq_epi32(_mm_srli_epi32(w1, 18), broadcomp));
}

/* we packed 128 15-bit values, touching 16 128-bit words, using 256 bytes */
static void filtereq15(const __m128i *in, u32 *matches, const INTEGER comp) {
  ...
  _mm_storeu_si128(
      ...
      _mm_cmpeq_epi32(_mm_and_si128(mask, _mm_srli_epi32(w0, 2)), broadcomp));
  _mm_storeu_si128(out + 31,
                   _mm_cmpeq_epi32(_mm_srli_epi32(w0, 17), broadcomp));
}

/* we packed 128 16-bit values, touching 17 128-bit words, using 272 bytes */
...
/* we packed 128 17-bit values, touching 18 128-bit words, using 288 bytes */
static void filtereq17(const __m128i *in, u32 *matches, const INTEGER comp) {
  ...
  _mm_storeu_si128(
      ...
      _mm_cmpeq_epi32(_mm_and_si128(mask, _mm_or_si128(_mm_srli_epi32(w1, 30),
                      ...
                      broadcomp));
  _mm_storeu_si128(out + 31,
                   _mm_cmpeq_epi32(_mm_srli_epi32(w0, 15), broadcomp));
}

/* we packed 128 18-bit values, touching 19 128-bit words, using 304 bytes */
static void filtereq18(const __m128i *in, u32 *matches, const INTEGER comp) {
  ...
  _mm_storeu_si128(
      ...
      _mm_cmpeq_epi32(_mm_and_si128(mask, _mm_or_si128(_mm_srli_epi32(w0, 28),
                      ...
                      broadcomp));
  _mm_storeu_si128(out + 31,
                   _mm_cmpeq_epi32(_mm_srli_epi32(w1, 14), broadcomp));
}

/* we packed 128 19-bit values, touching 20 128-bit words, using 320 bytes */
static void filtereq19(const __m128i *in, u32 *matches, const INTEGER comp) {
  ...
  _mm_storeu_si128(
      ...
      _mm_cmpeq_epi32(_mm_and_si128(mask, _mm_or_si128(_mm_srli_epi32(w1, 26),
                      ...
                      broadcomp));
  _mm_storeu_si128(out + 31,
                   _mm_cmpeq_epi32(_mm_srli_epi32(w0, 13), broadcomp));
}

/* we packed 128 20-bit values, touching 21 128-bit words, using 336 bytes */
static void filtereq20(const __m128i *in, u32 *matches, const INTEGER comp) {
  ...
  _mm_storeu_si128(
      ...
      _mm_cmpeq_epi32(_mm_and_si128(mask, _mm_or_si128(_mm_srli_epi32(w1, 24),
                      ...
                      broadcomp));
  _mm_storeu_si128(out + 23,
                   _mm_cmpeq_epi32(_mm_srli_epi32(w0, 12), broadcomp));
  w1 = _mm_lddqu_si128(in + 15);
  _mm_storeu_si128(out + 24,
                   _mm_cmpeq_epi32(_mm_and_si128(mask, w1), broadcomp));
  w0 = _mm_lddqu_si128(in + 16);
...
/* we packed 128 21-bit values, touching 22 128-bit words, using 352 bytes */
static void filtereq21(const __m128i *in, u32 *matches, const INTEGER comp) {
  ...
  _mm_storeu_si128(
      out + 25,
      _mm_cmpeq_epi32(_mm_and_si128(mask, _mm_or_si128(_mm_srli_epi32(w0, 13),
                                                       _mm_slli_epi32(w1, 19))),
                      broadcomp));
  _mm_storeu_si128(
      out + 26,
      _mm_cmpeq_epi32(_mm_and_si128(mask, _mm_srli_epi32(w1, 2)), broadcomp));
  ...
  _mm_storeu_si128(
      ...
      _mm_cmpeq_epi32(_mm_and_si128(mask, _mm_or_si128(_mm_srli_epi32(w1, 22),
                      ...
                      broadcomp));
  _mm_storeu_si128(out + 31,
                   _mm_cmpeq_epi32(_mm_srli_epi32(w0, 11), broadcomp));
}

/* we packed 128 22-bit values, touching 23 128-bit words, using 368 bytes */
static void filtereq22(const __m128i *in, u32 *matches, const INTEGER comp) {
  ...
  _mm_storeu_si128(
      ...
      _mm_cmpeq_epi32(_mm_and_si128(mask, _mm_or_si128(_mm_srli_epi32(w0, 20),
                      ...
                      broadcomp));
  _mm_storeu_si128(out + 31,
                   _mm_cmpeq_epi32(_mm_srli_epi32(w1, 10), broadcomp));
}

/* we packed 128 23-bit values, touching 24 128-bit words, using 384 bytes */
static void filtereq23(const __m128i *in, u32 *matches, const INTEGER comp) {
  ...
  _mm_storeu_si128(
      out + 27,
      _mm_cmpeq_epi32(_mm_and_si128(mask, _mm_or_si128(_mm_srli_epi32(w1, 13),
                                                       _mm_slli_epi32(w0, 19))),
                      broadcomp));
  _mm_storeu_si128(
      out + 28,
      _mm_cmpeq_epi32(_mm_and_si128(mask, _mm_srli_epi32(w0, 4)), broadcomp));
  ...
  _mm_storeu_si128(
      ...
      _mm_cmpeq_epi32(_mm_and_si128(mask, _mm_or_si128(_mm_srli_epi32(w0, 27),
                                                       _mm_slli_epi32(w1, 5))),
                      broadcomp));
  w0 = _mm_lddqu_si128(in + 22);
  _mm_storeu_si128(
      out + 30,
      _mm_cmpeq_epi32(_mm_and_si128(mask, _mm_or_si128(_mm_srli_epi32(w1, 18),
                                                       _mm_slli_epi32(w0, 14))),
                      broadcomp));
  _mm_storeu_si128(out + 31, _mm_cmpeq_epi32(_mm_srli_epi32(w0, 9), broadcomp));
}

/* we packed 128 24-bit values, touching 25 128-bit words, using 400 bytes */
static void filtereq24(const __m128i *in, u32 *matches, const INTEGER comp) {
  ...
  _mm_storeu_si128(
      ...
      _mm_cmpeq_epi32(_mm_and_si128(mask, _mm_or_si128(_mm_srli_epi32(w1, 16),
                                                       _mm_slli_epi32(w0, 16))),
                      broadcomp));
  _mm_storeu_si128(out + 19, _mm_cmpeq_epi32(_mm_srli_epi32(w0, 8), broadcomp));
  w1 = _mm_lddqu_si128(in + 15);
  _mm_storeu_si128(out + 20,
                   _mm_cmpeq_epi32(_mm_and_si128(mask, w1), broadcomp));
  w0 = _mm_lddqu_si128(in + 16);
  ...
  _mm_storeu_si128(
      ...
      _mm_cmpeq_epi32(_mm_and_si128(mask, _mm_or_si128(_mm_srli_epi32(w0, 16),
                                                       _mm_slli_epi32(w1, 16))),
                      broadcomp));
  _mm_storeu_si128(out + 23, _mm_cmpeq_epi32(_mm_srli_epi32(w1, 8), broadcomp));
  w0 = _mm_lddqu_si128(in + 18);
  _mm_storeu_si128(out + 24,
                   _mm_cmpeq_epi32(_mm_and_si128(mask, w0), broadcomp));
  w1 = _mm_lddqu_si128(in + 19);
  ...
  _mm_storeu_si128(
      ...
      _mm_cmpeq_epi32(_mm_and_si128(mask, _mm_or_si128(_mm_srli_epi32(w1, 16),
                                                       _mm_slli_epi32(w0, 16))),
                      broadcomp));
  _mm_storeu_si128(out + 27, _mm_cmpeq_epi32(_mm_srli_epi32(w0, 8), broadcomp));
  w1 = _mm_lddqu_si128(in + 21);
  _mm_storeu_si128(out + 28,
                   _mm_cmpeq_epi32(_mm_and_si128(mask, w1), broadcomp));
  w0 = _mm_lddqu_si128(in + 22);
  _mm_storeu_si128(
      ...
      _mm_cmpeq_epi32(_mm_and_si128(mask, _mm_or_si128(_mm_srli_epi32(w1, 24),
                                                       _mm_slli_epi32(w0, 8))),
                      broadcomp));
  w1 = _mm_lddqu_si128(in + 23);
  _mm_storeu_si128(
      out + 30,
      _mm_cmpeq_epi32(_mm_and_si128(mask, _mm_or_si128(_mm_srli_epi32(w0, 16),
                                                       _mm_slli_epi32(w1, 16))),
                      broadcomp));
  _mm_storeu_si128(out + 31, _mm_cmpeq_epi32(_mm_srli_epi32(w1, 8), broadcomp));
}

/* we packed 128 25-bit values, touching 26 128-bit words, using 416 bytes */
static void filtereq25(const __m128i *in, u32 *matches, const INTEGER comp) {
  ...
  _mm_storeu_si128(
      out + 25,
      _mm_cmpeq_epi32(_mm_and_si128(mask, _mm_or_si128(_mm_srli_epi32(w1, 17),
                                                       _mm_slli_epi32(w0, 15))),
                      broadcomp));
  w1 = _mm_lddqu_si128(in + 21);
  _mm_storeu_si128(
      out + 26,
  ...
  _mm_storeu_si128(
      ...
      _mm_cmpeq_epi32(_mm_and_si128(mask, _mm_or_si128(_mm_srli_epi32(w0, 21),
                                                       _mm_slli_epi32(w1, 11))),
                      broadcomp));
  w0 = _mm_lddqu_si128(in + 24);
  _mm_storeu_si128(
      out + 30,
      _mm_cmpeq_epi32(_mm_and_si128(mask, _mm_or_si128(_mm_srli_epi32(w1, 14),
                                                       _mm_slli_epi32(w0, 18))),
                      broadcomp));
  _mm_storeu_si128(out + 31, _mm_cmpeq_epi32(_mm_srli_epi32(w0, 7), broadcomp));
}

/* we packed 128 26-bit values, touching 27 128-bit words, using 432 bytes */
static void filtereq26(const __m128i *in, u32 *matches, const INTEGER comp) {
  ...
  _mm_storeu_si128(
      ...
      _mm_cmpeq_epi32(_mm_and_si128(mask, _mm_or_si128(_mm_srli_epi32(w1, 18),
                                                       _mm_slli_epi32(w0, 14))),
                      broadcomp));
  w1 = _mm_lddqu_si128(in + 25);
  _mm_storeu_si128(
      out + 30,
      _mm_cmpeq_epi32(_mm_and_si128(mask, _mm_or_si128(_mm_srli_epi32(w0, 12),
                                                       _mm_slli_epi32(w1, 20))),
                      broadcomp));
  _mm_storeu_si128(out + 31, _mm_cmpeq_epi32(_mm_srli_epi32(w1, 6), broadcomp));
}

/* we packed 128 27-bit values, touching 28 128-bit words, using 448 bytes */
static void filtereq27(const __m128i *in, u32 *matches, const INTEGER comp) {
  ...
  _mm_storeu_si128(
      out + 18,
      _mm_cmpeq_epi32(_mm_and_si128(mask, _mm_or_si128(_mm_srli_epi32(w1, 6),
                                                       _mm_slli_epi32(w0, 26))),
                      broadcomp));
  _mm_storeu_si128(
      out + 19,
      _mm_cmpeq_epi32(_mm_and_si128(mask, _mm_srli_epi32(w0, 1)), broadcomp));
  ...
  _mm_storeu_si128(
      ...
      _mm_cmpeq_epi32(_mm_and_si128(mask, _mm_or_si128(_mm_srli_epi32(w0, 15),
                                                       _mm_slli_epi32(w1, 17))),
                      broadcomp));
  w0 = _mm_lddqu_si128(in + 26);
  _mm_storeu_si128(
      out + 30,
      _mm_cmpeq_epi32(_mm_and_si128(mask, _mm_or_si128(_mm_srli_epi32(w1, 10),
                                                       _mm_slli_epi32(w0, 22))),
                      broadcomp));
  _mm_storeu_si128(out + 31, _mm_cmpeq_epi32(_mm_srli_epi32(w0, 5), broadcomp));
}

/* we packed 128 28-bit values, touching 29 128-bit words, using 464 bytes */
static void filtereq28(const __m128i *in, u32 *matches, const INTEGER comp) {
  ...
  _mm_storeu_si128(
      ...
      _mm_cmpeq_epi32(_mm_and_si128(mask, _mm_or_si128(_mm_srli_epi32(w1, 8),
                                                       _mm_slli_epi32(w0, 24))),
                      broadcomp));
  _mm_storeu_si128(out + 23, _mm_cmpeq_epi32(_mm_srli_epi32(w0, 4), broadcomp));
  w1 = _mm_lddqu_si128(in + 21);
  _mm_storeu_si128(out + 24,
                   _mm_cmpeq_epi32(_mm_and_si128(mask, w1), broadcomp));
  w0 = _mm_lddqu_si128(in + 22);
  ...
  _mm_storeu_si128(
      ...
      _mm_cmpeq_epi32(_mm_and_si128(mask, _mm_or_si128(_mm_srli_epi32(w1, 12),
                                                       _mm_slli_epi32(w0, 20))),
                      broadcomp));
  w1 = _mm_lddqu_si128(in + 27);
  _mm_storeu_si128(
      out + 30,
      _mm_cmpeq_epi32(_mm_and_si128(mask, _mm_or_si128(_mm_srli_epi32(w0, 8),
                                                       _mm_slli_epi32(w1, 24))),
                      broadcomp));
  _mm_storeu_si128(out + 31, _mm_cmpeq_epi32(_mm_srli_epi32(w1, 4), broadcomp));
}

/* we packed 128 29-bit values, touching 30 128-bit words, using 480 bytes */
static void filtereq29(const __m128i *in, u32 *matches, const INTEGER comp) {
  ...
  _mm_storeu_si128(
      out + 19,
      _mm_cmpeq_epi32(_mm_and_si128(mask, _mm_or_si128(_mm_srli_epi32(w1, 7),
                                                       _mm_slli_epi32(w0, 25))),
                      broadcomp));
  w1 = _mm_lddqu_si128(in + 19);
  _mm_storeu_si128(
      out + 20,
      _mm_cmpeq_epi32(_mm_and_si128(mask, _mm_or_si128(_mm_srli_epi32(w0, 4),
                                                       _mm_slli_epi32(w1, 28))),
                      broadcomp));
  _mm_storeu_si128(
      out + 21,
      _mm_cmpeq_epi32(_mm_and_si128(mask, _mm_srli_epi32(w1, 1)), broadcomp));
  ...
  _mm_storeu_si128(
      out + 29,
      _mm_cmpeq_epi32(_mm_and_si128(mask, _mm_or_si128(_mm_srli_epi32(w0, 9),
                                                       _mm_slli_epi32(w1, 23))),
                      broadcomp));
  w0 = _mm_lddqu_si128(in + 28);
  _mm_storeu_si128(
      out + 30,
      _mm_cmpeq_epi32(_mm_and_si128(mask, _mm_or_si128(_mm_srli_epi32(w1, 6),
                                                       _mm_slli_epi32(w0, 26))),
                      broadcomp));
  _mm_storeu_si128(out + 31, _mm_cmpeq_epi32(_mm_srli_epi32(w0, 3), broadcomp));
}

/* we packed 128 30-bit values, touching 31 128-bit words, using 496 bytes */
static void filtereq30(const __m128i *in, u32 *matches, const INTEGER comp) {
  ...
  _mm_storeu_si128(
      ...
      _mm_cmpeq_epi32(_mm_and_si128(mask, _mm_or_si128(_mm_srli_epi32(w1, 6),
                                                       _mm_slli_epi32(w0, 26))),
                      broadcomp));
  w1 = _mm_lddqu_si128(in + 29);
  _mm_storeu_si128(
      out + 30,
      _mm_cmpeq_epi32(_mm_and_si128(mask, _mm_or_si128(_mm_srli_epi32(w0, 4),
                                                       _mm_slli_epi32(w1, 28))),
                      broadcomp));
  _mm_storeu_si128(out + 31, _mm_cmpeq_epi32(_mm_srli_epi32(w1, 2), broadcomp));
}

/* we packed 128 31-bit values, touching 32 128-bit words, using 512 bytes */
static void filtereq31(const __m128i *in, u32 *matches, const INTEGER comp) {
  ...
  _mm_storeu_si128(
      out + 29,
      _mm_cmpeq_epi32(_mm_and_si128(mask, _mm_or_si128(_mm_srli_epi32(w0, 3),
                                                       _mm_slli_epi32(w1, 29))),
                      broadcomp));
  w0 = _mm_lddqu_si128(in + 30);
  _mm_storeu_si128(
      out + 30,
      _mm_cmpeq_epi32(_mm_and_si128(mask, _mm_or_si128(_mm_srli_epi32(w1, 2),
                                                       _mm_slli_epi32(w0, 30))),
                      broadcomp));
  _mm_storeu_si128(out + 31, _mm_cmpeq_epi32(_mm_srli_epi32(w0, 1), broadcomp));
}

/* we packed 128 32-bit values, touching 33 128-bit words, using 528 bytes */
static void filtereq32(const __m128i *in, u32 *matches, const INTEGER comp) {
  ...
  w0 = _mm_lddqu_si128(in + 16);
  _mm_storeu_si128(out + 16, _mm_cmpeq_epi32(w0, broadcomp));
  w1 = _mm_lddqu_si128(in + 17);
  _mm_storeu_si128(out + 17, _mm_cmpeq_epi32(w1, broadcomp));
  w0 = _mm_lddqu_si128(in + 18);
  _mm_storeu_si128(out + 18, _mm_cmpeq_epi32(w0, broadcomp));
  w1 = _mm_lddqu_si128(in + 19);
  _mm_storeu_si128(out + 19, _mm_cmpeq_epi32(w1, broadcomp));
  w0 = _mm_lddqu_si128(in + 20);
  _mm_storeu_si128(out + 20, _mm_cmpeq_epi32(w0, broadcomp));
  w1 = _mm_lddqu_si128(in + 21);
  _mm_storeu_si128(out + 21, _mm_cmpeq_epi32(w1, broadcomp));
  w0 = _mm_lddqu_si128(in + 22);
  _mm_storeu_si128(out + 22, _mm_cmpeq_epi32(w0, broadcomp));
  w1 = _mm_lddqu_si128(in + 23);
  _mm_storeu_si128(out + 23, _mm_cmpeq_epi32(w1, broadcomp));
  w0 = _mm_lddqu_si128(in + 24);
  _mm_storeu_si128(out + 24, _mm_cmpeq_epi32(w0, broadcomp));
  w1 = _mm_lddqu_si128(in + 25);
  _mm_storeu_si128(out + 25, _mm_cmpeq_epi32(w1, broadcomp));
  w0 = _mm_lddqu_si128(in + 26);
  _mm_storeu_si128(out + 26, _mm_cmpeq_epi32(w0, broadcomp));
  w1 = _mm_lddqu_si128(in + 27);
  _mm_storeu_si128(out + 27, _mm_cmpeq_epi32(w1, broadcomp));
  w0 = _mm_lddqu_si128(in + 28);
  _mm_storeu_si128(out + 28, _mm_cmpeq_epi32(w0, broadcomp));
  w1 = _mm_lddqu_si128(in + 29);
  _mm_storeu_si128(out + 29, _mm_cmpeq_epi32(w1, broadcomp));
  w0 = _mm_lddqu_si128(in + 30);
  _mm_storeu_si128(out + 30, _mm_cmpeq_epi32(w0, broadcomp));
  w1 = _mm_lddqu_si128(in + 31);
  _mm_storeu_si128(out + 31, _mm_cmpeq_epi32(w1, broadcomp));
}
...
static void filterneq1(const __m128i *in, u32 *matches, const INTEGER comp) {
  ...
  _mm_storeu_si128(
      out + 19,
      _mm_xor_si128(_mm_cmpeq_epi32(_mm_and_si128(mask, _mm_srli_epi32(w0, 19)),
                                    broadcomp),
                    _mm_set1_epi32(-1)));
  _mm_storeu_si128(
      out + 20,
  ...
  _mm_storeu_si128(
      out + 23,
      _mm_xor_si128(_mm_cmpeq_epi32(_mm_and_si128(mask, _mm_srli_epi32(w0, 23)),
                                    broadcomp),
                    _mm_set1_epi32(-1)));
  _mm_storeu_si128(
      out + 24,
  ...
  _mm_storeu_si128(
      out + 29,
      _mm_xor_si128(_mm_cmpeq_epi32(_mm_and_si128(mask, _mm_srli_epi32(w0, 29)),
                                    broadcomp),
                    _mm_set1_epi32(-1)));
  _mm_storeu_si128(
      out + 30,
      _mm_xor_si128(_mm_cmpeq_epi32(_mm_and_si128(mask, _mm_srli_epi32(w0, 30)),
                                    broadcomp),
                    _mm_set1_epi32(-1)));
  _mm_storeu_si128(
      out + 31,
      _mm_xor_si128(_mm_cmpeq_epi32(_mm_srli_epi32(w0, 31), broadcomp),
                    _mm_set1_epi32(-1)));
}
...
static void filterneq2(const __m128i *in, u32 *matches, const INTEGER comp) {
  ...
  _mm_storeu_si128(
      out + 29,
      _mm_xor_si128(_mm_cmpeq_epi32(_mm_and_si128(mask, _mm_srli_epi32(w1, 26)),
                                    broadcomp),
                    _mm_set1_epi32(-1)));
  _mm_storeu_si128(
      out + 30,
      _mm_xor_si128(_mm_cmpeq_epi32(_mm_and_si128(mask, _mm_srli_epi32(w1, 28)),
                                    broadcomp),
                    _mm_set1_epi32(-1)));
  _mm_storeu_si128(
      out + 31,
      _mm_xor_si128(_mm_cmpeq_epi32(_mm_srli_epi32(w1, 30), broadcomp),
                    _mm_set1_epi32(-1)));
}
...
static void filterneq3(const __m128i *in, u32 *matches, const INTEGER comp) {
  ...
  _mm_storeu_si128(
      out + 17,
      _mm_xor_si128(_mm_cmpeq_epi32(_mm_and_si128(mask, _mm_srli_epi32(w1, 19)),
                                    broadcomp),
                    _mm_set1_epi32(-1)));
  _mm_storeu_si128(
      out + 18,
  ...
  _mm_storeu_si128(
      out + 19,
      _mm_xor_si128(_mm_cmpeq_epi32(_mm_and_si128(mask, _mm_srli_epi32(w1, 25)),
                                    broadcomp),
                    _mm_set1_epi32(-1)));
  _mm_storeu_si128(
      out + 20,
  ...
  _mm_storeu_si128(
      out + 31,
      _mm_xor_si128(_mm_cmpeq_epi32(_mm_srli_epi32(w0, 29), broadcomp),
                    _mm_set1_epi32(-1)));
}
...
static void filterneq4(const __m128i *in, u32 *matches, const INTEGER comp) {
  ...
  _mm_storeu_si128(
      out + 23,
      _mm_xor_si128(_mm_cmpeq_epi32(_mm_srli_epi32(w0, 28), broadcomp),
                    _mm_set1_epi32(-1)));
  w1 = _mm_lddqu_si128(in + 3);
  _mm_storeu_si128(
      out + 24,
...
static void filterneq5(const __m128i *in, u32 *matches, const INTEGER comp) {
  ...
  _mm_storeu_si128(
      out + 31,
      _mm_xor_si128(_mm_cmpeq_epi32(_mm_srli_epi32(w0, 27), broadcomp),
                    _mm_set1_epi32(-1)));
}
...
static void filterneq6(const __m128i *in, u32 *matches, const INTEGER comp) {
  ...
  _mm_storeu_si128(
      out + 31,
      _mm_xor_si128(_mm_cmpeq_epi32(_mm_srli_epi32(w1, 26), broadcomp),
                    _mm_set1_epi32(-1)));
}
...
static void filterneq7(const __m128i *in, u32 *matches, const INTEGER comp) {
  ...
  _mm_storeu_si128(
      out + 25,
      _mm_xor_si128(_mm_cmpeq_epi32(_mm_and_si128(mask, _mm_srli_epi32(w1, 15)),
                                    broadcomp),
                    _mm_set1_epi32(-1)));
  _mm_storeu_si128(
      out + 26,
  ...
  _mm_storeu_si128(
      out + 31,
      _mm_xor_si128(_mm_cmpeq_epi32(_mm_srli_epi32(w0, 25), broadcomp),
                    _mm_set1_epi32(-1)));
}
...
static void filterneq9(const __m128i *in, u32 *matches, const INTEGER comp) {
  ...
  _mm_storeu_si128(
      out + 31,
      _mm_xor_si128(_mm_cmpeq_epi32(_mm_srli_epi32(w0, 23), broadcomp),
                    _mm_set1_epi32(-1)));
}
...
static void filterneq10(const __m128i *in, u32 *matches, const INTEGER comp) {
  ...
  _mm_storeu_si128(
      out + 31,
      _mm_xor_si128(_mm_cmpeq_epi32(_mm_srli_epi32(w1, 22), broadcomp),
                    _mm_set1_epi32(-1)));
}
...
static void filterneq11(const __m128i *in, u32 *matches, const INTEGER comp) {
  ...
  _mm_storeu_si128(
      out + 31,
      _mm_xor_si128(_mm_cmpeq_epi32(_mm_srli_epi32(w0, 21), broadcomp),
                    _mm_set1_epi32(-1)));
}
...
static void filterneq12(const __m128i *in, u32 *matches, const INTEGER comp) {
  ...
  _mm_storeu_si128(
      out + 23,
      _mm_xor_si128(_mm_cmpeq_epi32(_mm_srli_epi32(w0, 20), broadcomp),
                    _mm_set1_epi32(-1)));
  w1 = _mm_lddqu_si128(in + 9);
  _mm_storeu_si128(
      out + 24,
...
static void filterneq13(const __m128i *in, u32 *matches, const INTEGER comp) {
  ...
  _mm_storeu_si128(
      out + 31,
      _mm_xor_si128(_mm_cmpeq_epi32(_mm_srli_epi32(w0, 19), broadcomp),
                    _mm_set1_epi32(-1)));
}
...
static void filterneq14(const __m128i *in, u32 *matches, const INTEGER comp) {
  ...
  _mm_storeu_si128(
      out + 31,
      _mm_xor_si128(_mm_cmpeq_epi32(_mm_srli_epi32(w1, 18), broadcomp),
                    _mm_set1_epi32(-1)));
}
...
static void filterneq15(const __m128i *in, u32 *matches, const INTEGER comp) {
  ...
  _mm_storeu_si128(
      out + 31,
      _mm_xor_si128(_mm_cmpeq_epi32(_mm_srli_epi32(w0, 17), broadcomp),
                    _mm_set1_epi32(-1)));
}
...
static void filterneq17(const __m128i *in, u32 *matches, const INTEGER comp) {
  ...
  _mm_storeu_si128(
      out + 31,
      _mm_xor_si128(_mm_cmpeq_epi32(_mm_srli_epi32(w0, 15), broadcomp),
                    _mm_set1_epi32(-1)));
}
...
static void filterneq18(const __m128i *in, u32 *matches, const INTEGER comp) {
  ...
  _mm_storeu_si128(
      out + 31,
      _mm_xor_si128(_mm_cmpeq_epi32(_mm_srli_epi32(w1, 14), broadcomp),
                    _mm_set1_epi32(-1)));
}
...
static void filterneq19(const __m128i *in, u32 *matches, const INTEGER comp) {
  ...
  _mm_storeu_si128(
      out + 31,
      _mm_xor_si128(_mm_cmpeq_epi32(_mm_srli_epi32(w0, 13), broadcomp),
                    _mm_set1_epi32(-1)));
}
...
static void filterneq20(const __m128i *in, u32 *matches, const INTEGER comp) {
  ...
  _mm_storeu_si128(
      out + 23,
      _mm_xor_si128(_mm_cmpeq_epi32(_mm_srli_epi32(w0, 12), broadcomp),
                    _mm_set1_epi32(-1)));
  w1 = _mm_lddqu_si128(in + 15);
  _mm_storeu_si128(
      out + 24,
...
static void filterneq21(const __m128i *in, u32 *matches, const INTEGER comp) {
  ...
  _mm_storeu_si128(
      ...
      _mm_xor_si128(
          _mm_cmpeq_epi32(
              _mm_and_si128(mask, _mm_or_si128(_mm_srli_epi32(w0, 13),
                                               _mm_slli_epi32(w1, 19))),
              broadcomp),
          _mm_set1_epi32(-1)));
  _mm_storeu_si128(
  ...
  _mm_storeu_si128(
      out + 31,
      _mm_xor_si128(_mm_cmpeq_epi32(_mm_srli_epi32(w0, 11), broadcomp),
                    _mm_set1_epi32(-1)));
}
...
static void filterneq22(const __m128i *in, u32 *matches, const INTEGER comp) {
  ...
  _mm_storeu_si128(
      out + 31,
      _mm_xor_si128(_mm_cmpeq_epi32(_mm_srli_epi32(w1, 10), broadcomp),
                    _mm_set1_epi32(-1)));
}
...
static void filterneq23(const __m128i *in, u32 *matches, const INTEGER comp) {
  ...
  _mm_storeu_si128(
      ...
      _mm_xor_si128(
          _mm_cmpeq_epi32(
              _mm_and_si128(mask, _mm_or_si128(_mm_srli_epi32(w1, 13),
                                               _mm_slli_epi32(w0, 19))),
              broadcomp),
          _mm_set1_epi32(-1)));
  _mm_storeu_si128(
  ...
  _mm_storeu_si128(
      ...
      _mm_xor_si128(
          ...
          _mm_set1_epi32(-1)));
  _mm_storeu_si128(
      out + 31, _mm_xor_si128(_mm_cmpeq_epi32(_mm_srli_epi32(w0, 9), broadcomp),
                              _mm_set1_epi32(-1)));
}
...
static void filterneq24(const __m128i *in, u32 *matches, const INTEGER comp) {
  ...
  _mm_storeu_si128(
      ...
      _mm_xor_si128(
          ...
          _mm_set1_epi32(-1)));
  _mm_storeu_si128(
      out + 19, _mm_xor_si128(_mm_cmpeq_epi32(_mm_srli_epi32(w0, 8), broadcomp),
                              _mm_set1_epi32(-1)));
  w1 = _mm_lddqu_si128(in + 15);
  _mm_storeu_si128(
      out + 20,
  ...
  _mm_storeu_si128(
      ...
      _mm_xor_si128(
          ...
          _mm_set1_epi32(-1)));
  _mm_storeu_si128(
      out + 23, _mm_xor_si128(_mm_cmpeq_epi32(_mm_srli_epi32(w1, 8), broadcomp),
                              _mm_set1_epi32(-1)));
  w0 = _mm_lddqu_si128(in + 18);
  _mm_storeu_si128(
      out + 24,
  ...
  _mm_storeu_si128(
      ...
      _mm_xor_si128(
          ...
          _mm_set1_epi32(-1)));
  _mm_storeu_si128(
      out + 27, _mm_xor_si128(_mm_cmpeq_epi32(_mm_srli_epi32(w0, 8), broadcomp),
                              _mm_set1_epi32(-1)));
  w1 = _mm_lddqu_si128(in + 21);
  _mm_storeu_si128(
      out + 28,
  ...
  _mm_storeu_si128(
      ...
      _mm_xor_si128(
          ...
          _mm_set1_epi32(-1)));
  _mm_storeu_si128(
      out + 31, _mm_xor_si128(_mm_cmpeq_epi32(_mm_srli_epi32(w1, 8), broadcomp),
                              _mm_set1_epi32(-1)));
}
...
static void filterneq25(const __m128i *in, u32 *matches, const INTEGER comp) {
  ...
  _mm_storeu_si128(
      ...
      _mm_xor_si128(
          _mm_cmpeq_epi32(
              _mm_and_si128(mask, _mm_or_si128(_mm_srli_epi32(w1, 17),
                                               _mm_slli_epi32(w0, 15))),
              broadcomp),
          _mm_set1_epi32(-1)));
  w1 = _mm_lddqu_si128(in + 21);
  ...
  _mm_storeu_si128(
      ...
      _mm_xor_si128(
          ...
          _mm_set1_epi32(-1)));
  _mm_storeu_si128(
      out + 31, _mm_xor_si128(_mm_cmpeq_epi32(_mm_srli_epi32(w0, 7), broadcomp),
                              _mm_set1_epi32(-1)));
}
...
static void filterneq26(const __m128i *in, u32 *matches, const INTEGER comp) {
  ...
  _mm_storeu_si128(
      ...
      _mm_xor_si128(
          ...
          _mm_set1_epi32(-1)));
  _mm_storeu_si128(
      out + 31, _mm_xor_si128(_mm_cmpeq_epi32(_mm_srli_epi32(w1, 6), broadcomp),
                              _mm_set1_epi32(-1)));
}
...
static void filterneq27(const __m128i *in, u32 *matches, const INTEGER comp) {
  ...
  _mm_storeu_si128(
      ...
      _mm_xor_si128(
          _mm_cmpeq_epi32(
              _mm_and_si128(mask, _mm_or_si128(_mm_srli_epi32(w1, 6),
                                               _mm_slli_epi32(w0, 26))),
              broadcomp),
          _mm_set1_epi32(-1)));
  _mm_storeu_si128(
  ...
  _mm_storeu_si128(
      ...
      _mm_xor_si128(
          ...
          _mm_set1_epi32(-1)));
  _mm_storeu_si128(
      out + 31, _mm_xor_si128(_mm_cmpeq_epi32(_mm_srli_epi32(w0, 5), broadcomp),
                              _mm_set1_epi32(-1)));
}
...
static void filterneq28(const __m128i *in, u32 *matches, const INTEGER comp) {
  ...
  _mm_storeu_si128(
      ...
      _mm_xor_si128(
          ...
          _mm_set1_epi32(-1)));
  _mm_storeu_si128(
      out + 23, _mm_xor_si128(_mm_cmpeq_epi32(_mm_srli_epi32(w0, 4), broadcomp),
                              _mm_set1_epi32(-1)));
  w1 = _mm_lddqu_si128(in + 21);
  _mm_storeu_si128(
      out + 24,
  ...
  _mm_storeu_si128(
      ...
      _mm_xor_si128(
          ...
          _mm_set1_epi32(-1)));
  _mm_storeu_si128(
      out + 31, _mm_xor_si128(_mm_cmpeq_epi32(_mm_srli_epi32(w1, 4), broadcomp),
                              _mm_set1_epi32(-1)));
}
...
static void filterneq29(const __m128i *in, u32 *matches, const INTEGER comp) {
  ...
  _mm_storeu_si128(
      ...
      _mm_xor_si128(
          _mm_cmpeq_epi32(
              _mm_and_si128(mask, _mm_or_si128(_mm_srli_epi32(w1, 7),
                                               _mm_slli_epi32(w0, 25))),
              broadcomp),
          _mm_set1_epi32(-1)));
  w1 = _mm_lddqu_si128(in + 19);
  _mm_storeu_si128(
      ...
      _mm_xor_si128(
          _mm_cmpeq_epi32(
              _mm_and_si128(mask, _mm_or_si128(_mm_srli_epi32(w0, 4),
                                               _mm_slli_epi32(w1, 28))),
              broadcomp),
          _mm_set1_epi32(-1)));
  _mm_storeu_si128(
  ...
  _mm_storeu_si128(
      ...
      _mm_xor_si128(
          _mm_cmpeq_epi32(
              _mm_and_si128(mask, _mm_or_si128(_mm_srli_epi32(w0, 9),
                                               _mm_slli_epi32(w1, 23))),
              broadcomp),
          _mm_set1_epi32(-1)));
  w0 = _mm_lddqu_si128(in + 28);
  _mm_storeu_si128(
      ...
      _mm_xor_si128(
          ...
          _mm_set1_epi32(-1)));
  _mm_storeu_si128(
      out + 31, _mm_xor_si128(_mm_cmpeq_epi32(_mm_srli_epi32(w0, 3), broadcomp),
                              _mm_set1_epi32(-1)));
}
...
static void filterneq30(const __m128i *in, u32 *matches, const INTEGER comp) {
  ...
  _mm_storeu_si128(
      ...
      _mm_xor_si128(
          ...
          _mm_set1_epi32(-1)));
  _mm_storeu_si128(
      out + 31, _mm_xor_si128(_mm_cmpeq_epi32(_mm_srli_epi32(w1, 2), broadcomp),
                              _mm_set1_epi32(-1)));
}
...
static void filterneq31(const __m128i *in, u32 *matches, const INTEGER comp) {
  ...
  _mm_storeu_si128(
      ...
      _mm_xor_si128(
          _mm_cmpeq_epi32(
              _mm_and_si128(mask, _mm_or_si128(_mm_srli_epi32(w0, 3),
                                               _mm_slli_epi32(w1, 29))),
              broadcomp),
          _mm_set1_epi32(-1)));
  w0 = _mm_lddqu_si128(in + 30);
  _mm_storeu_si128(
      ...
      _mm_xor_si128(
          _mm_cmpeq_epi32(
              _mm_and_si128(mask, _mm_or_si128(_mm_srli_epi32(w1, 2),
                                               _mm_slli_epi32(w0, 30))),
              broadcomp),
          _mm_set1_epi32(-1)));
  _mm_storeu_si128(
      out + 31, _mm_xor_si128(_mm_cmpeq_epi32(_mm_srli_epi32(w0, 1), broadcomp),
                              _mm_set1_epi32(-1)));
}
...
static void filterneq32(const __m128i *in, u32 *matches, const INTEGER comp) {
  ...
  _mm_storeu_si128(out + 15, _mm_xor_si128(_mm_cmpeq_epi32(w1, broadcomp),
                                           _mm_set1_epi32(-1)));
  w0 = _mm_lddqu_si128(in + 16);
  _mm_storeu_si128(out + 16, _mm_xor_si128(_mm_cmpeq_epi32(w0, broadcomp),
                                           _mm_set1_epi32(-1)));
  w1 = _mm_lddqu_si128(in + 17);
  _mm_storeu_si128(out + 17, _mm_xor_si128(_mm_cmpeq_epi32(w1, broadcomp),
                                           _mm_set1_epi32(-1)));
  w0 = _mm_lddqu_si128(in + 18);
  _mm_storeu_si128(out + 18, _mm_xor_si128(_mm_cmpeq_epi32(w0, broadcomp),
                                           _mm_set1_epi32(-1)));
  w1 = _mm_lddqu_si128(in + 19);
  _mm_storeu_si128(out + 19, _mm_xor_si128(_mm_cmpeq_epi32(w1, broadcomp),
                                           _mm_set1_epi32(-1)));
  w0 = _mm_lddqu_si128(in + 20);
  _mm_storeu_si128(out + 20, _mm_xor_si128(_mm_cmpeq_epi32(w0, broadcomp),
                                           _mm_set1_epi32(-1)));
  w1 = _mm_lddqu_si128(in + 21);
  _mm_storeu_si128(out + 21, _mm_xor_si128(_mm_cmpeq_epi32(w1, broadcomp),
                                           _mm_set1_epi32(-1)));
  w0 = _mm_lddqu_si128(in + 22);
  _mm_storeu_si128(out + 22, _mm_xor_si128(_mm_cmpeq_epi32(w0, broadcomp),
                                           _mm_set1_epi32(-1)));
  w1 = _mm_lddqu_si128(in + 23);
  _mm_storeu_si128(out + 23, _mm_xor_si128(_mm_cmpeq_epi32(w1, broadcomp),
                                           _mm_set1_epi32(-1)));
  w0 = _mm_lddqu_si128(in + 24);
  _mm_storeu_si128(out + 24, _mm_xor_si128(_mm_cmpeq_epi32(w0, broadcomp),
                                           _mm_set1_epi32(-1)));
  w1 = _mm_lddqu_si128(in + 25);
  _mm_storeu_si128(out + 25, _mm_xor_si128(_mm_cmpeq_epi32(w1, broadcomp),
                                           _mm_set1_epi32(-1)));
  w0 = _mm_lddqu_si128(in + 26);
  _mm_storeu_si128(out + 26, _mm_xor_si128(_mm_cmpeq_epi32(w0, broadcomp),
                                           _mm_set1_epi32(-1)));
  w1 = _mm_lddqu_si128(in + 27);
  _mm_storeu_si128(out + 27, _mm_xor_si128(_mm_cmpeq_epi32(w1, broadcomp),
                                           _mm_set1_epi32(-1)));
  w0 = _mm_lddqu_si128(in + 28);
  _mm_storeu_si128(out + 28, _mm_xor_si128(_mm_cmpeq_epi32(w0, broadcomp),
                                           _mm_set1_epi32(-1)));
  w1 = _mm_lddqu_si128(in + 29);
  _mm_storeu_si128(out + 29, _mm_xor_si128(_mm_cmpeq_epi32(w1, broadcomp),
                                           _mm_set1_epi32(-1)));
  w0 = _mm_lddqu_si128(in + 30);
  _mm_storeu_si128(out + 30, _mm_xor_si128(_mm_cmpeq_epi32(w0, broadcomp),
                                           _mm_set1_epi32(-1)));
  w1 = _mm_lddqu_si128(in + 31);
  _mm_storeu_si128(out + 31, _mm_xor_si128(_mm_cmpeq_epi32(w1, broadcomp),
                                           _mm_set1_epi32(-1)));
}
...
static void filtergt1(const __m128i *in, u32 *matches, const INTEGER comp) {
  ...
  _mm_storeu_si128(
      out + 19,
      _mm_cmpgt_epi32(_mm_and_si128(mask, _mm_srli_epi32(w0, 19)), broadcomp));
  _mm_storeu_si128(
      out + 20,
  ...
  _mm_storeu_si128(
      out + 23,
      _mm_cmpgt_epi32(_mm_and_si128(mask, _mm_srli_epi32(w0, 23)), broadcomp));
  _mm_storeu_si128(
      out + 24,
  ...
  _mm_storeu_si128(
      out + 29,
      _mm_cmpgt_epi32(_mm_and_si128(mask, _mm_srli_epi32(w0, 29)), broadcomp));
  _mm_storeu_si128(
      out + 30,
      _mm_cmpgt_epi32(_mm_and_si128(mask, _mm_srli_epi32(w0, 30)), broadcomp));
  _mm_storeu_si128(out + 31,
                   _mm_cmpgt_epi32(_mm_srli_epi32(w0, 31), broadcomp));
}
...
static void filtergt2(const __m128i *in, u32 *matches, const INTEGER comp) {
  ...
  _mm_storeu_si128(
      out + 29,
      _mm_cmpgt_epi32(_mm_and_si128(mask, _mm_srli_epi32(w1, 26)), broadcomp));
  _mm_storeu_si128(
      out + 30,
      _mm_cmpgt_epi32(_mm_and_si128(mask, _mm_srli_epi32(w1, 28)), broadcomp));
  _mm_storeu_si128(out + 31,
                   _mm_cmpgt_epi32(_mm_srli_epi32(w1, 30), broadcomp));
}
...
static void filtergt3(const __m128i *in, u32 *matches, const INTEGER comp) {
  ...
  _mm_storeu_si128(
      out + 17,
      _mm_cmpgt_epi32(_mm_and_si128(mask, _mm_srli_epi32(w1, 19)), broadcomp));
  _mm_storeu_si128(
      out + 18,
      _mm_cmpgt_epi32(_mm_and_si128(mask, _mm_srli_epi32(w1, 22)), broadcomp));
  _mm_storeu_si128(
      out + 19,
      _mm_cmpgt_epi32(_mm_and_si128(mask, _mm_srli_epi32(w1, 25)), broadcomp));
  _mm_storeu_si128(
      out + 20,
      _mm_cmpgt_epi32(_mm_and_si128(mask, _mm_srli_epi32(w1, 28)), broadcomp));
  w0 = _mm_lddqu_si128(in + 2);
  ...
  _mm_storeu_si128(
      ...
      _mm_cmpgt_epi32(_mm_and_si128(mask, _mm_srli_epi32(w0, 26)), broadcomp));
  _mm_storeu_si128(out + 31,
                   _mm_cmpgt_epi32(_mm_srli_epi32(w0, 29), broadcomp));
}
...
static void filtergt4(const __m128i *in, u32 *matches, const INTEGER comp) {
  ...
  _mm_storeu_si128(
      ...
      _mm_cmpgt_epi32(_mm_and_si128(mask, _mm_srli_epi32(w0, 24)), broadcomp));
  _mm_storeu_si128(out + 23,
                   _mm_cmpgt_epi32(_mm_srli_epi32(w0, 28), broadcomp));
  w1 = _mm_lddqu_si128(in + 3);
  _mm_storeu_si128(out + 24,
                   _mm_cmpgt_epi32(_mm_and_si128(mask, w1), broadcomp));
  _mm_storeu_si128(
...
static void filtergt5(const __m128i *in, u32 *matches, const INTEGER comp) {
  ...
  _mm_storeu_si128(
      ...
      _mm_cmpgt_epi32(_mm_and_si128(mask, _mm_srli_epi32(w0, 22)), broadcomp));
  _mm_storeu_si128(out + 31,
                   _mm_cmpgt_epi32(_mm_srli_epi32(w0, 27), broadcomp));
}
...
static void filtergt6(const __m128i *in, u32 *matches, const INTEGER comp) {
  ...
  _mm_storeu_si128(
      ...
      _mm_cmpgt_epi32(_mm_and_si128(mask, _mm_srli_epi32(w1, 20)), broadcomp));
  _mm_storeu_si128(out + 31,
                   _mm_cmpgt_epi32(_mm_srli_epi32(w1, 26), broadcomp));
}
...
static void filtergt7(const __m128i *in, u32 *matches, const INTEGER comp) {
  ...
  _mm_storeu_si128(
      out + 25,
      _mm_cmpgt_epi32(_mm_and_si128(mask, _mm_srli_epi32(w1, 15)), broadcomp));
  _mm_storeu_si128(
      out + 26,
      _mm_cmpgt_epi32(_mm_and_si128(mask, _mm_srli_epi32(w1, 22)), broadcomp));
  w0 = _mm_lddqu_si128(in + 6);
  ...
  _mm_storeu_si128(
      ...
      _mm_cmpgt_epi32(_mm_and_si128(mask, _mm_srli_epi32(w0, 18)), broadcomp));
  _mm_storeu_si128(out + 31,
                   _mm_cmpgt_epi32(_mm_srli_epi32(w0, 25), broadcomp));
}
...
static void filtergt9(const __m128i *in, u32 *matches, const INTEGER comp) {
  ...
  _mm_storeu_si128(
      ...
      _mm_cmpgt_epi32(_mm_and_si128(mask, _mm_srli_epi32(w0, 14)), broadcomp));
  _mm_storeu_si128(out + 31,
                   _mm_cmpgt_epi32(_mm_srli_epi32(w0, 23), broadcomp));
}
...
static void filtergt10(const __m128i *in, u32 *matches, const INTEGER comp) {
  ...
  _mm_storeu_si128(
      ...
      _mm_cmpgt_epi32(_mm_and_si128(mask, _mm_srli_epi32(w1, 12)), broadcomp));
  _mm_storeu_si128(out + 31,
                   _mm_cmpgt_epi32(_mm_srli_epi32(w1, 22), broadcomp));
}
...
static void filtergt11(const __m128i *in, u32 *matches, const INTEGER comp) {
  ...
  _mm_storeu_si128(
      ...
      _mm_cmpgt_epi32(_mm_and_si128(mask, _mm_srli_epi32(w0, 10)), broadcomp));
  _mm_storeu_si128(out + 31,
                   _mm_cmpgt_epi32(_mm_srli_epi32(w0, 21), broadcomp));
}
...
static void filtergt12(const __m128i *in, u32 *matches, const INTEGER comp) {
  ...
  _mm_storeu_si128(
      ...
      _mm_cmpgt_epi32(_mm_and_si128(mask, _mm_srli_epi32(w0, 8)), broadcomp));
  _mm_storeu_si128(out + 23,
                   _mm_cmpgt_epi32(_mm_srli_epi32(w0, 20), broadcomp));
  w1 = _mm_lddqu_si128(in + 9);
  _mm_storeu_si128(out + 24,
                   _mm_cmpgt_epi32(_mm_and_si128(mask, w1), broadcomp));
  _mm_storeu_si128(
...
static void filtergt13(const __m128i *in, u32 *matches, const INTEGER comp) {
  ...
  _mm_storeu_si128(
      ...
      _mm_cmpgt_epi32(_mm_and_si128(mask, _mm_srli_epi32(w0, 6)), broadcomp));
  _mm_storeu_si128(out + 31,
                   _mm_cmpgt_epi32(_mm_srli_epi32(w0, 19), broadcomp));
}
...
static void filtergt14(const __m128i *in, u32 *matches, const INTEGER comp) {
  ...
  _mm_storeu_si128(
      ...
      _mm_cmpgt_epi32(_mm_and_si128(mask, _mm_srli_epi32(w1, 4)), broadcomp));
  _mm_storeu_si128(out + 31,
                   _mm_cmpgt_epi32(_mm_srli_epi32(w1, 18), broadcomp));
}
...
static void filtergt15(const __m128i *in, u32 *matches, const INTEGER comp) {
  ...
  _mm_storeu_si128(
      ...
      _mm_cmpgt_epi32(_mm_and_si128(mask, _mm_srli_epi32(w0, 2)), broadcomp));
  _mm_storeu_si128(out + 31,
                   _mm_cmpgt_epi32(_mm_srli_epi32(w0, 17), broadcomp));
}
...
static void filtergt17(const __m128i *in, u32 *matches, const INTEGER comp) {
  ...
  _mm_storeu_si128(
      ...
      _mm_cmpgt_epi32(_mm_and_si128(mask, _mm_or_si128(_mm_srli_epi32(w1, 30),
                      ...
                      broadcomp));
  _mm_storeu_si128(out + 31,
                   _mm_cmpgt_epi32(_mm_srli_epi32(w0, 15), broadcomp));
}
...
static void filtergt18(const __m128i *in, u32 *matches, const INTEGER comp) {
  ...
  _mm_storeu_si128(
      ...
      _mm_cmpgt_epi32(_mm_and_si128(mask, _mm_or_si128(_mm_srli_epi32(w0, 28),
                      ...
                      broadcomp));
  _mm_storeu_si128(out + 31,
                   _mm_cmpgt_epi32(_mm_srli_epi32(w1, 14), broadcomp));
}
...
static void filtergt19(const __m128i *in, u32 *matches, const INTEGER comp) {
  ...
  _mm_storeu_si128(
      ...
      _mm_cmpgt_epi32(_mm_and_si128(mask, _mm_or_si128(_mm_srli_epi32(w1, 26),
                      ...
                      broadcomp));
  _mm_storeu_si128(out + 31,
                   _mm_cmpgt_epi32(_mm_srli_epi32(w0, 13), broadcomp));
}
...
static void filtergt20(const __m128i *in, u32 *matches, const INTEGER comp) {
  ...
  _mm_storeu_si128(
      ...
      _mm_cmpgt_epi32(_mm_and_si128(mask, _mm_or_si128(_mm_srli_epi32(w1, 24),
                      ...
                      broadcomp));
  _mm_storeu_si128(out + 23,
                   _mm_cmpgt_epi32(_mm_srli_epi32(w0, 12), broadcomp));
  w1 = _mm_lddqu_si128(in + 15);
  _mm_storeu_si128(out + 24,
                   _mm_cmpgt_epi32(_mm_and_si128(mask, w1), broadcomp));
  w0 = _mm_lddqu_si128(in + 16);
...
static void filtergt21(const __m128i *in, u32 *matches, const INTEGER comp) {
  ...
  _mm_storeu_si128(
      out + 25,
      _mm_cmpgt_epi32(_mm_and_si128(mask, _mm_or_si128(_mm_srli_epi32(w0, 13),
                                                       _mm_slli_epi32(w1, 19))),
                      broadcomp));
  _mm_storeu_si128(
      out + 26,
  ...
  _mm_storeu_si128(
      ...
      _mm_cmpgt_epi32(_mm_and_si128(mask, _mm_or_si128(_mm_srli_epi32(w1, 22),
                      ...
                      broadcomp));
  _mm_storeu_si128(out + 31,
                   _mm_cmpgt_epi32(_mm_srli_epi32(w0, 11), broadcomp));
}
...
static void filtergt22(const __m128i *in, u32 *matches, const INTEGER comp) {
  ...
  _mm_storeu_si128(
      ...
      _mm_cmpgt_epi32(_mm_and_si128(mask, _mm_or_si128(_mm_srli_epi32(w0, 20),
                      ...
                      broadcomp));
  _mm_storeu_si128(out + 31,
                   _mm_cmpgt_epi32(_mm_srli_epi32(w1, 10), broadcomp));
}
...
static void filtergt23(const __m128i *in, u32 *matches, const INTEGER comp) {
  ...
  _mm_storeu_si128(
      out + 27,
      _mm_cmpgt_epi32(_mm_and_si128(mask, _mm_or_si128(_mm_srli_epi32(w1, 13),
                                                       _mm_slli_epi32(w0, 19))),
                      broadcomp));
  _mm_storeu_si128(
      out + 28,
  ...
  _mm_storeu_si128(
      ...
      _mm_cmpgt_epi32(_mm_and_si128(mask, _mm_or_si128(_mm_srli_epi32(w1, 18),
                                                       _mm_slli_epi32(w0, 14))),
                      broadcomp));
  _mm_storeu_si128(out + 31, _mm_cmpgt_epi32(_mm_srli_epi32(w0, 9), broadcomp));
}
...
static void filtergt24(const __m128i *in, u32 *matches, const INTEGER comp) {
  ...
  _mm_storeu_si128(
      ...
      _mm_cmpgt_epi32(_mm_and_si128(mask, _mm_or_si128(_mm_srli_epi32(w1, 16),
                                                       _mm_slli_epi32(w0, 16))),
                      broadcomp));
  _mm_storeu_si128(out + 19, _mm_cmpgt_epi32(_mm_srli_epi32(w0, 8), broadcomp));
  w1 = _mm_lddqu_si128(in + 15);
  _mm_storeu_si128(out + 20,
                   _mm_cmpgt_epi32(_mm_and_si128(mask, w1), broadcomp));
  w0 = _mm_lddqu_si128(in + 16);
  ...
  _mm_storeu_si128(
      ...
      _mm_cmpgt_epi32(_mm_and_si128(mask, _mm_or_si128(_mm_srli_epi32(w0, 16),
                                                       _mm_slli_epi32(w1, 16))),
                      broadcomp));
  _mm_storeu_si128(out + 23, _mm_cmpgt_epi32(_mm_srli_epi32(w1, 8), broadcomp));
  w0 = _mm_lddqu_si128(in + 18);
  _mm_storeu_si128(out + 24,
                   _mm_cmpgt_epi32(_mm_and_si128(mask, w0), broadcomp));
  w1 = _mm_lddqu_si128(in + 19);
  ...
  _mm_storeu_si128(
      ...
      _mm_cmpgt_epi32(_mm_and_si128(mask, _mm_or_si128(_mm_srli_epi32(w1, 16),
                                                       _mm_slli_epi32(w0, 16))),
                      broadcomp));
  _mm_storeu_si128(out + 27, _mm_cmpgt_epi32(_mm_srli_epi32(w0, 8), broadcomp));
  w1 = _mm_lddqu_si128(in + 21);
  _mm_storeu_si128(out + 28,
                   _mm_cmpgt_epi32(_mm_and_si128(mask, w1), broadcomp));
  w0 = _mm_lddqu_si128(in + 22);
  ...
  _mm_storeu_si128(
      ...
      _mm_cmpgt_epi32(_mm_and_si128(mask, _mm_or_si128(_mm_srli_epi32(w0, 16),
                                                       _mm_slli_epi32(w1, 16))),
                      broadcomp));
  _mm_storeu_si128(out + 31, _mm_cmpgt_epi32(_mm_srli_epi32(w1, 8), broadcomp));
}
...
static void filtergt25(const __m128i *in, u32 *matches, const INTEGER comp) {
  ...
  _mm_storeu_si128(
      out + 25,
      _mm_cmpgt_epi32(_mm_and_si128(mask, _mm_or_si128(_mm_srli_epi32(w1, 17),
                                                       _mm_slli_epi32(w0, 15))),
                      broadcomp));
  w1 = _mm_lddqu_si128(in + 21);
  _mm_storeu_si128(
  ...
  _mm_storeu_si128(
      ...
      _mm_cmpgt_epi32(_mm_and_si128(mask, _mm_or_si128(_mm_srli_epi32(w1, 14),
                                                       _mm_slli_epi32(w0, 18))),
                      broadcomp));
  _mm_storeu_si128(out + 31, _mm_cmpgt_epi32(_mm_srli_epi32(w0, 7), broadcomp));
}
...
static void filtergt26(const __m128i *in, u32 *matches, const INTEGER comp) {
  ...
  _mm_storeu_si128(
      ...
      _mm_cmpgt_epi32(_mm_and_si128(mask, _mm_or_si128(_mm_srli_epi32(w0, 12),
                                                       _mm_slli_epi32(w1, 20))),
                      broadcomp));
  _mm_storeu_si128(out + 31, _mm_cmpgt_epi32(_mm_srli_epi32(w1, 6), broadcomp));
}
...
static void filtergt27(const __m128i *in, u32 *matches, const INTEGER comp) {
  ...
  _mm_storeu_si128(
      out + 18,
      _mm_cmpgt_epi32(_mm_and_si128(mask, _mm_or_si128(_mm_srli_epi32(w1, 6),
                                                       _mm_slli_epi32(w0, 26))),
                      broadcomp));
  _mm_storeu_si128(
      out + 19,
  ...
  _mm_storeu_si128(
      ...
      _mm_cmpgt_epi32(_mm_and_si128(mask, _mm_or_si128(_mm_srli_epi32(w1, 10),
                                                       _mm_slli_epi32(w0, 22))),
                      broadcomp));
  _mm_storeu_si128(out + 31, _mm_cmpgt_epi32(_mm_srli_epi32(w0, 5), broadcomp));
}
...
static void filtergt28(const __m128i *in, u32 *matches, const INTEGER comp) {
  ...
  _mm_storeu_si128(
      ...
      _mm_cmpgt_epi32(_mm_and_si128(mask, _mm_or_si128(_mm_srli_epi32(w1, 8),
                                                       _mm_slli_epi32(w0, 24))),
                      broadcomp));
  _mm_storeu_si128(out + 23, _mm_cmpgt_epi32(_mm_srli_epi32(w0, 4), broadcomp));
  w1 = _mm_lddqu_si128(in + 21);
  _mm_storeu_si128(out + 24,
                   _mm_cmpgt_epi32(_mm_and_si128(mask, w1), broadcomp));
  w0 = _mm_lddqu_si128(in + 22);
  ...
  _mm_storeu_si128(
      ...
      _mm_cmpgt_epi32(_mm_and_si128(mask, _mm_or_si128(_mm_srli_epi32(w0, 8),
                                                       _mm_slli_epi32(w1, 24))),
                      broadcomp));
  _mm_storeu_si128(out + 31, _mm_cmpgt_epi32(_mm_srli_epi32(w1, 4), broadcomp));
}
...
static void filtergt29(const __m128i *in, u32 *matches, const INTEGER comp) {
  ...
  _mm_storeu_si128(
      out + 19,
      _mm_cmpgt_epi32(_mm_and_si128(mask, _mm_or_si128(_mm_srli_epi32(w1, 7),
                                                       _mm_slli_epi32(w0, 25))),
                      broadcomp));
  w1 = _mm_lddqu_si128(in + 19);
  _mm_storeu_si128(
      out + 20,
      _mm_cmpgt_epi32(_mm_and_si128(mask, _mm_or_si128(_mm_srli_epi32(w0, 4),
                                                       _mm_slli_epi32(w1, 28))),
                      broadcomp));
  _mm_storeu_si128(
      out + 21,
  ...
  _mm_storeu_si128(
      out + 29,
      _mm_cmpgt_epi32(_mm_and_si128(mask, _mm_or_si128(_mm_srli_epi32(w0, 9),
                                                       _mm_slli_epi32(w1, 23))),
                      broadcomp));
  w0 = _mm_lddqu_si128(in + 28);
  _mm_storeu_si128(
      ...
      _mm_cmpgt_epi32(_mm_and_si128(mask, _mm_or_si128(_mm_srli_epi32(w1, 6),
                                                       _mm_slli_epi32(w0, 26))),
                      broadcomp));
  _mm_storeu_si128(out + 31, _mm_cmpgt_epi32(_mm_srli_epi32(w0, 3), broadcomp));
}
...
static void filtergt30(const __m128i *in, u32 *matches, const INTEGER comp) {
  ...
  _mm_storeu_si128(
      ...
      _mm_cmpgt_epi32(_mm_and_si128(mask, _mm_or_si128(_mm_srli_epi32(w0, 4),
                                                       _mm_slli_epi32(w1, 28))),
                      broadcomp));
  _mm_storeu_si128(out + 31, _mm_cmpgt_epi32(_mm_srli_epi32(w1, 2), broadcomp));
}
...
static void filtergt31(const __m128i *in, u32 *matches, const INTEGER comp) {
  ...
  _mm_storeu_si128(
      out + 29,
      _mm_cmpgt_epi32(_mm_and_si128(mask, _mm_or_si128(_mm_srli_epi32(w0, 3),
                                                       _mm_slli_epi32(w1, 29))),
                      broadcomp));
  w0 = _mm_lddqu_si128(in + 30);
  _mm_storeu_si128(
      out + 30,
      _mm_cmpgt_epi32(_mm_and_si128(mask, _mm_or_si128(_mm_srli_epi32(w1, 2),
                                                       _mm_slli_epi32(w0, 30))),
                      broadcomp));
  _mm_storeu_si128(out + 31, _mm_cmpgt_epi32(_mm_srli_epi32(w0, 1), broadcomp));
}
...
static void filtergt32(const __m128i *in, u32 *matches, const INTEGER comp) {
  ...
  w0 = _mm_lddqu_si128(in + 16);
  _mm_storeu_si128(out + 16, _mm_cmpgt_epi32(w0, broadcomp));
  w1 = _mm_lddqu_si128(in + 17);
  _mm_storeu_si128(out + 17, _mm_cmpgt_epi32(w1, broadcomp));
  w0 = _mm_lddqu_si128(in + 18);
  _mm_storeu_si128(out + 18, _mm_cmpgt_epi32(w0, broadcomp));
  w1 = _mm_lddqu_si128(in + 19);
  _mm_storeu_si128(out + 19, _mm_cmpgt_epi32(w1, broadcomp));
  w0 = _mm_lddqu_si128(in + 20);
  _mm_storeu_si128(out + 20, _mm_cmpgt_epi32(w0, broadcomp));
  w1 = _mm_lddqu_si128(in + 21);
  _mm_storeu_si128(out + 21, _mm_cmpgt_epi32(w1, broadcomp));
  w0 = _mm_lddqu_si128(in + 22);
  _mm_storeu_si128(out + 22, _mm_cmpgt_epi32(w0, broadcomp));
  w1 = _mm_lddqu_si128(in + 23);
  _mm_storeu_si128(out + 23, _mm_cmpgt_epi32(w1, broadcomp));
  w0 = _mm_lddqu_si128(in + 24);
  _mm_storeu_si128(out + 24, _mm_cmpgt_epi32(w0, broadcomp));
  w1 = _mm_lddqu_si128(in + 25);
  _mm_storeu_si128(out + 25, _mm_cmpgt_epi32(w1, broadcomp));
  w0 = _mm_lddqu_si128(in + 26);
  _mm_storeu_si128(out + 26, _mm_cmpgt_epi32(w0, broadcomp));
  w1 = _mm_lddqu_si128(in + 27);
  _mm_storeu_si128(out + 27, _mm_cmpgt_epi32(w1, broadcomp));
  w0 = _mm_lddqu_si128(in + 28);
  _mm_storeu_si128(out + 28, _mm_cmpgt_epi32(w0, broadcomp));
  w1 = _mm_lddqu_si128(in + 29);
  _mm_storeu_si128(out + 29, _mm_cmpgt_epi32(w1, broadcomp));
  w0 = _mm_lddqu_si128(in + 30);
  _mm_storeu_si128(out + 30, _mm_cmpgt_epi32(w0, broadcomp));
  w1 = _mm_lddqu_si128(in + 31);
  _mm_storeu_si128(out + 31, _mm_cmpgt_epi32(w1, broadcomp));
}
...
static void filterlt1(const __m128i *in, u32 *matches, const INTEGER comp) {
  ...
  _mm_storeu_si128(
      out + 19,
      _mm_cmpgt_epi32(broadcomp, _mm_and_si128(mask, _mm_srli_epi32(w0, 19))));
  _mm_storeu_si128(
      out + 20,
  ...
  _mm_storeu_si128(
      out + 23,
      _mm_cmpgt_epi32(broadcomp, _mm_and_si128(mask, _mm_srli_epi32(w0, 23))));
  _mm_storeu_si128(
      out + 24,
  ...
  _mm_storeu_si128(
      out + 29,
      _mm_cmpgt_epi32(broadcomp, _mm_and_si128(mask, _mm_srli_epi32(w0, 29))));
  _mm_storeu_si128(
      out + 30,
      _mm_cmpgt_epi32(broadcomp, _mm_and_si128(mask, _mm_srli_epi32(w0, 30))));
  _mm_storeu_si128(out + 31,
                   _mm_cmpgt_epi32(broadcomp, _mm_srli_epi32(w0, 31)));
}
...
static void filterlt2(const __m128i *in, u32 *matches, const INTEGER comp) {
  ...
  _mm_storeu_si128(
      out + 29,
      _mm_cmpgt_epi32(broadcomp, _mm_and_si128(mask, _mm_srli_epi32(w1, 26))));
  _mm_storeu_si128(
      out + 30,
      _mm_cmpgt_epi32(broadcomp, _mm_and_si128(mask, _mm_srli_epi32(w1, 28))));
  _mm_storeu_si128(out + 31,
                   _mm_cmpgt_epi32(broadcomp, _mm_srli_epi32(w1, 30)));
}
...
static void filterlt3(const __m128i *in, u32 *matches, const INTEGER comp) {
  ...
  _mm_storeu_si128(
      out + 17,
      _mm_cmpgt_epi32(broadcomp, _mm_and_si128(mask, _mm_srli_epi32(w1, 19))));
  _mm_storeu_si128(
      out + 18,
      _mm_cmpgt_epi32(broadcomp, _mm_and_si128(mask, _mm_srli_epi32(w1, 22))));
  _mm_storeu_si128(
      out + 19,
      _mm_cmpgt_epi32(broadcomp, _mm_and_si128(mask, _mm_srli_epi32(w1, 25))));
  _mm_storeu_si128(
      out + 20,
      _mm_cmpgt_epi32(broadcomp, _mm_and_si128(mask, _mm_srli_epi32(w1, 28))));
  w0 = _mm_lddqu_si128(in + 2);
  ...
  _mm_storeu_si128(
      ...
      _mm_cmpgt_epi32(broadcomp, _mm_and_si128(mask, _mm_srli_epi32(w0, 26))));
  _mm_storeu_si128(out + 31,
                   _mm_cmpgt_epi32(broadcomp, _mm_srli_epi32(w0, 29)));
}
...
static void filterlt4(const __m128i *in, u32 *matches, const INTEGER comp) {
  ...
  _mm_storeu_si128(
      ...
      _mm_cmpgt_epi32(broadcomp, _mm_and_si128(mask, _mm_srli_epi32(w0, 24))));
  _mm_storeu_si128(out + 23,
                   _mm_cmpgt_epi32(broadcomp, _mm_srli_epi32(w0, 28)));
  w1 = _mm_lddqu_si128(in + 3);
  _mm_storeu_si128(out + 24,
                   _mm_cmpgt_epi32(broadcomp, _mm_and_si128(mask, w1)));
  _mm_storeu_si128(
...
static void filterlt5(const __m128i *in, u32 *matches, const INTEGER comp) {
  ...
  _mm_storeu_si128(
      ...
      _mm_cmpgt_epi32(broadcomp, _mm_and_si128(mask, _mm_srli_epi32(w0, 22))));
  _mm_storeu_si128(out + 31,
                   _mm_cmpgt_epi32(broadcomp, _mm_srli_epi32(w0, 27)));
}
...
static void filterlt6(const __m128i *in, u32 *matches, const INTEGER comp) {
  ...
  _mm_storeu_si128(
      ...
      _mm_cmpgt_epi32(broadcomp, _mm_and_si128(mask, _mm_srli_epi32(w1, 20))));
  _mm_storeu_si128(out + 31,
                   _mm_cmpgt_epi32(broadcomp, _mm_srli_epi32(w1, 26)));
}
...
static void filterlt7(const __m128i *in, u32 *matches, const INTEGER comp) {
  ...
  _mm_storeu_si128(
      out + 25,
      _mm_cmpgt_epi32(broadcomp, _mm_and_si128(mask, _mm_srli_epi32(w1, 15))));
  _mm_storeu_si128(
      out + 26,
      _mm_cmpgt_epi32(broadcomp, _mm_and_si128(mask, _mm_srli_epi32(w1, 22))));
  w0 = _mm_lddqu_si128(in + 6);
  ...
  _mm_storeu_si128(
      ...
      _mm_cmpgt_epi32(broadcomp, _mm_and_si128(mask, _mm_srli_epi32(w0, 18))));
  _mm_storeu_si128(out + 31,
                   _mm_cmpgt_epi32(broadcomp, _mm_srli_epi32(w0, 25)));
}
...
static void filterlt9(const __m128i *in, u32 *matches, const INTEGER comp) {
  ...
  _mm_storeu_si128(
      ...
      _mm_cmpgt_epi32(broadcomp, _mm_and_si128(mask, _mm_srli_epi32(w0, 14))));
  _mm_storeu_si128(out + 31,
                   _mm_cmpgt_epi32(broadcomp, _mm_srli_epi32(w0, 23)));
}
...
static void filterlt10(const __m128i *in, u32 *matches, const INTEGER comp) {
  ...
  _mm_storeu_si128(
      ...
      _mm_cmpgt_epi32(broadcomp, _mm_and_si128(mask, _mm_srli_epi32(w1, 12))));
  _mm_storeu_si128(out + 31,
                   _mm_cmpgt_epi32(broadcomp, _mm_srli_epi32(w1, 22)));
}
...
static void filterlt11(const __m128i *in, u32 *matches, const INTEGER comp) {
  ...
  _mm_storeu_si128(
      ...
      _mm_cmpgt_epi32(broadcomp, _mm_and_si128(mask, _mm_srli_epi32(w0, 10))));
  _mm_storeu_si128(out + 31,
                   _mm_cmpgt_epi32(broadcomp, _mm_srli_epi32(w0, 21)));
}
...
static void filterlt12(const __m128i *in, u32 *matches, const INTEGER comp) {
  ...
  _mm_storeu_si128(
      ...
      _mm_cmpgt_epi32(broadcomp, _mm_and_si128(mask, _mm_srli_epi32(w0, 8))));
  _mm_storeu_si128(out + 23,
                   _mm_cmpgt_epi32(broadcomp, _mm_srli_epi32(w0, 20)));
  w1 = _mm_lddqu_si128(in + 9);
  _mm_storeu_si128(out + 24,
                   _mm_cmpgt_epi32(broadcomp, _mm_and_si128(mask, w1)));
  _mm_storeu_si128(
...
static void filterlt13(const __m128i *in, u32 *matches, const INTEGER comp) {
  ...
  _mm_storeu_si128(
      ...
      _mm_cmpgt_epi32(broadcomp, _mm_and_si128(mask, _mm_srli_epi32(w0, 6))));
  _mm_storeu_si128(out + 31,
                   _mm_cmpgt_epi32(broadcomp, _mm_srli_epi32(w0, 19)));
}
...
static void filterlt14(const __m128i *in, u32 *matches, const INTEGER comp) {
  ...
  _mm_storeu_si128(
      ...
      _mm_cmpgt_epi32(broadcomp, _mm_and_si128(mask, _mm_srli_epi32(w1, 4))));
  _mm_storeu_si128(out + 31,
                   _mm_cmpgt_epi32(broadcomp, _mm_srli_epi32(w1, 18)));
}
...
static void filterlt15(const __m128i *in, u32 *matches, const INTEGER comp) {
  ...
  _mm_storeu_si128(
      ...
      _mm_cmpgt_epi32(broadcomp, _mm_and_si128(mask, _mm_srli_epi32(w0, 2))));
  _mm_storeu_si128(out + 31,
                   _mm_cmpgt_epi32(broadcomp, _mm_srli_epi32(w0, 17)));
}
  ...
  _mm_storeu_si128(
      ...
      _mm_cmpgt_epi32(
          broadcomp, _mm_and_si128(mask, _mm_or_si128(_mm_srli_epi32(w1, 30),
                                                      _mm_slli_epi32(w0, 2)))));
  _mm_storeu_si128(out + 31,
                   _mm_cmpgt_epi32(broadcomp, _mm_srli_epi32(w0, 15)));
}
  ...
  _mm_storeu_si128(
      ...
      _mm_cmpgt_epi32(
          broadcomp, _mm_and_si128(mask, _mm_or_si128(_mm_srli_epi32(w0, 28),
                                                      _mm_slli_epi32(w1, 4)))));
  _mm_storeu_si128(out + 31,
                   _mm_cmpgt_epi32(broadcomp, _mm_srli_epi32(w1, 14)));
}
  ...
  _mm_storeu_si128(
      ...
      _mm_cmpgt_epi32(
          broadcomp, _mm_and_si128(mask, _mm_or_si128(_mm_srli_epi32(w1, 26),
                                                      _mm_slli_epi32(w0, 6)))));
  _mm_storeu_si128(out + 31,
                   _mm_cmpgt_epi32(broadcomp, _mm_srli_epi32(w0, 13)));
}
  ...
  _mm_storeu_si128(
      ...
      _mm_cmpgt_epi32(
          broadcomp, _mm_and_si128(mask, _mm_or_si128(_mm_srli_epi32(w1, 24),
                                                      _mm_slli_epi32(w0, 8)))));
  _mm_storeu_si128(out + 23,
                   _mm_cmpgt_epi32(broadcomp, _mm_srli_epi32(w0, 12)));
  w1 = _mm_lddqu_si128(in + 15);
  _mm_storeu_si128(out + 24,
                   _mm_cmpgt_epi32(broadcomp, _mm_and_si128(mask, w1)));
  w0 = _mm_lddqu_si128(in + 16);
  _mm_storeu_si128(
      out + 25, _mm_cmpgt_epi32(
                    broadcomp,
                    _mm_and_si128(mask, _mm_or_si128(_mm_srli_epi32(w1, 20),
                                                     _mm_slli_epi32(w0, 12)))));
  _mm_storeu_si128(
...
static void filterlt21(const __m128i *in, u32 *matches, const INTEGER comp) {
  ...
  w0 = _mm_lddqu_si128(in + 20);
  _mm_storeu_si128(
      out + 30, _mm_cmpgt_epi32(
                    broadcomp,
                    _mm_and_si128(mask, _mm_or_si128(_mm_srli_epi32(w1, 22),
                                                     _mm_slli_epi32(w0, 10)))));
  _mm_storeu_si128(out + 31,
                   _mm_cmpgt_epi32(broadcomp, _mm_srli_epi32(w0, 11)));
}
  ...
  _mm_storeu_si128(
      out + 30, _mm_cmpgt_epi32(
                    ...
                    _mm_and_si128(mask, _mm_or_si128(_mm_srli_epi32(w0, 20),
                                                     _mm_slli_epi32(w1, 12)))));
  _mm_storeu_si128(out + 31,
                   _mm_cmpgt_epi32(broadcomp, _mm_srli_epi32(w1, 10)));
}
...
static void filterlt23(const __m128i *in, u32 *matches, const INTEGER comp) {
  ...
  _mm_storeu_si128(
      out + 30, _mm_cmpgt_epi32(
                    ...
                    _mm_and_si128(mask, _mm_or_si128(_mm_srli_epi32(w1, 18),
                                                     _mm_slli_epi32(w0, 14)))));
  _mm_storeu_si128(out + 31, _mm_cmpgt_epi32(broadcomp, _mm_srli_epi32(w0, 9)));
}
...
static void filterlt24(const __m128i *in, u32 *matches, const INTEGER comp) {
  ...
  _mm_storeu_si128(
      out + 18, _mm_cmpgt_epi32(
                    ...
                    _mm_and_si128(mask, _mm_or_si128(_mm_srli_epi32(w1, 16),
                                                     _mm_slli_epi32(w0, 16)))));
  _mm_storeu_si128(out + 19, _mm_cmpgt_epi32(broadcomp, _mm_srli_epi32(w0, 8)));
  w1 = _mm_lddqu_si128(in + 15);
  _mm_storeu_si128(out + 20,
                   _mm_cmpgt_epi32(broadcomp, _mm_and_si128(mask, w1)));
  w0 = _mm_lddqu_si128(in + 16);
  ...
  _mm_storeu_si128(
      out + 22, _mm_cmpgt_epi32(
                    ...
                    _mm_and_si128(mask, _mm_or_si128(_mm_srli_epi32(w0, 16),
                                                     _mm_slli_epi32(w1, 16)))));
  _mm_storeu_si128(out + 23, _mm_cmpgt_epi32(broadcomp, _mm_srli_epi32(w1, 8)));
  w0 = _mm_lddqu_si128(in + 18);
  _mm_storeu_si128(out + 24,
                   _mm_cmpgt_epi32(broadcomp, _mm_and_si128(mask, w0)));
  w1 = _mm_lddqu_si128(in + 19);
  ...
  _mm_storeu_si128(
      out + 26, _mm_cmpgt_epi32(
                    ...
                    _mm_and_si128(mask, _mm_or_si128(_mm_srli_epi32(w1, 16),
                                                     _mm_slli_epi32(w0, 16)))));
  _mm_storeu_si128(out + 27, _mm_cmpgt_epi32(broadcomp, _mm_srli_epi32(w0, 8)));
  w1 = _mm_lddqu_si128(in + 21);
  _mm_storeu_si128(out + 28,
                   _mm_cmpgt_epi32(broadcomp, _mm_and_si128(mask, w1)));
  w0 = _mm_lddqu_si128(in + 22);
  ...
  _mm_storeu_si128(
      out + 30, _mm_cmpgt_epi32(
                    ...
                    _mm_and_si128(mask, _mm_or_si128(_mm_srli_epi32(w0, 16),
                                                     _mm_slli_epi32(w1, 16)))));
  _mm_storeu_si128(out + 31, _mm_cmpgt_epi32(broadcomp, _mm_srli_epi32(w1, 8)));
}
...
static void filterlt25(const __m128i *in, u32 *matches, const INTEGER comp) {
  ...
  _mm_storeu_si128(
      out + 30, _mm_cmpgt_epi32(
                    ...
                    _mm_and_si128(mask, _mm_or_si128(_mm_srli_epi32(w1, 14),
                                                     _mm_slli_epi32(w0, 18)))));
  _mm_storeu_si128(out + 31, _mm_cmpgt_epi32(broadcomp, _mm_srli_epi32(w0, 7)));
}
...
static void filterlt26(const __m128i *in, u32 *matches, const INTEGER comp) {
  ...
  _mm_storeu_si128(
      out + 30, _mm_cmpgt_epi32(
                    ...
                    _mm_and_si128(mask, _mm_or_si128(_mm_srli_epi32(w0, 12),
                                                     _mm_slli_epi32(w1, 20)))));
  _mm_storeu_si128(out + 31, _mm_cmpgt_epi32(broadcomp, _mm_srli_epi32(w1, 6)));
}
...
static void filterlt27(const __m128i *in, u32 *matches, const INTEGER comp) {
  ...
  _mm_storeu_si128(
      out + 30, _mm_cmpgt_epi32(
                    ...
                    _mm_and_si128(mask, _mm_or_si128(_mm_srli_epi32(w1, 10),
                                                     _mm_slli_epi32(w0, 22)))));
  _mm_storeu_si128(out + 31, _mm_cmpgt_epi32(broadcomp, _mm_srli_epi32(w0, 5)));
}
...
static void filterlt28(const __m128i *in, u32 *matches, const INTEGER comp) {
  ...
  _mm_storeu_si128(
      out + 22, _mm_cmpgt_epi32(
                    ...
                    _mm_and_si128(mask, _mm_or_si128(_mm_srli_epi32(w1, 8),
                                                     _mm_slli_epi32(w0, 24)))));
  _mm_storeu_si128(out + 23, _mm_cmpgt_epi32(broadcomp, _mm_srli_epi32(w0, 4)));
  w1 = _mm_lddqu_si128(in + 21);
  _mm_storeu_si128(out + 24,
                   _mm_cmpgt_epi32(broadcomp, _mm_and_si128(mask, w1)));
  w0 = _mm_lddqu_si128(in + 22);
  ...
  _mm_storeu_si128(
      out + 30, _mm_cmpgt_epi32(
                    ...
                    _mm_and_si128(mask, _mm_or_si128(_mm_srli_epi32(w0, 8),
                                                     _mm_slli_epi32(w1, 24)))));
  _mm_storeu_si128(out + 31, _mm_cmpgt_epi32(broadcomp, _mm_srli_epi32(w1, 4)));
}
...
static void filterlt29(const __m128i *in, u32 *matches, const INTEGER comp) {
  ...
  _mm_storeu_si128(
      out + 30, _mm_cmpgt_epi32(
                    ...
                    _mm_and_si128(mask, _mm_or_si128(_mm_srli_epi32(w1, 6),
                                                     _mm_slli_epi32(w0, 26)))));
  _mm_storeu_si128(out + 31, _mm_cmpgt_epi32(broadcomp, _mm_srli_epi32(w0, 3)));
}
...
static void filterlt30(const __m128i *in, u32 *matches, const INTEGER comp) {
  ...
  _mm_storeu_si128(
      out + 30, _mm_cmpgt_epi32(
                    ...
                    _mm_and_si128(mask, _mm_or_si128(_mm_srli_epi32(w0, 4),
                                                     _mm_slli_epi32(w1, 28)))));
  _mm_storeu_si128(out + 31, _mm_cmpgt_epi32(broadcomp, _mm_srli_epi32(w1, 2)));
}
...
static void filterlt31(const __m128i *in, u32 *matches, const INTEGER comp) {
  ...
  _mm_storeu_si128(
      out + 30, _mm_cmpgt_epi32(
                    ...
                    _mm_and_si128(mask, _mm_or_si128(_mm_srli_epi32(w1, 2),
                                                     _mm_slli_epi32(w0, 30)))));
  _mm_storeu_si128(out + 31, _mm_cmpgt_epi32(broadcomp, _mm_srli_epi32(w0, 1)));
}
...
static void filterlt32(const __m128i *in, u32 *matches, const INTEGER comp) {
  ...
  w0 = _mm_lddqu_si128(in + 16);
  _mm_storeu_si128(out + 16, _mm_cmpgt_epi32(broadcomp, w0));
  w1 = _mm_lddqu_si128(in + 17);
  _mm_storeu_si128(out + 17, _mm_cmpgt_epi32(broadcomp, w1));
  w0 = _mm_lddqu_si128(in + 18);
  _mm_storeu_si128(out + 18, _mm_cmpgt_epi32(broadcomp, w0));
  w1 = _mm_lddqu_si128(in + 19);
  _mm_storeu_si128(out + 19, _mm_cmpgt_epi32(broadcomp, w1));
  w0 = _mm_lddqu_si128(in + 20);
  _mm_storeu_si128(out + 20, _mm_cmpgt_epi32(broadcomp, w0));
  w1 = _mm_lddqu_si128(in + 21);
  _mm_storeu_si128(out + 21, _mm_cmpgt_epi32(broadcomp, w1));
  w0 = _mm_lddqu_si128(in + 22);
  _mm_storeu_si128(out + 22, _mm_cmpgt_epi32(broadcomp, w0));
  w1 = _mm_lddqu_si128(in + 23);
  _mm_storeu_si128(out + 23, _mm_cmpgt_epi32(broadcomp, w1));
  w0 = _mm_lddqu_si128(in + 24);
  _mm_storeu_si128(out + 24, _mm_cmpgt_epi32(broadcomp, w0));
  w1 = _mm_lddqu_si128(in + 25);
  _mm_storeu_si128(out + 25, _mm_cmpgt_epi32(broadcomp, w1));
  w0 = _mm_lddqu_si128(in + 26);
  _mm_storeu_si128(out + 26, _mm_cmpgt_epi32(broadcomp, w0));
  w1 = _mm_lddqu_si128(in + 27);
  _mm_storeu_si128(out + 27, _mm_cmpgt_epi32(broadcomp, w1));
  w0 = _mm_lddqu_si128(in + 28);
  _mm_storeu_si128(out + 28, _mm_cmpgt_epi32(broadcomp, w0));
  w1 = _mm_lddqu_si128(in + 29);
  _mm_storeu_si128(out + 29, _mm_cmpgt_epi32(broadcomp, w1));
  w0 = _mm_lddqu_si128(in + 30);
  _mm_storeu_si128(out + 30, _mm_cmpgt_epi32(broadcomp, w0));
  w1 = _mm_lddqu_si128(in + 31);
  _mm_storeu_si128(out + 31, _mm_cmpgt_epi32(broadcomp, w1));
}
