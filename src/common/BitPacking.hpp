...
class BitPacking {
  public:
    ...
    static u8 packFixed(const INTEGER* src, u8* dest, const u32 size, INTEGER diff) {
    ...
    /// @param diff The difference between min and max in the source data. 
    /// @return The amount of bits used to pack the data.
    static u8 packArbitrary(const INTEGER* src, u8* dest, const u32 size, INTEGER diff) {
      auto pack_size = packSizeFor(diff);
  ...
  private:
    ...
    static void compress(const INTEGER* src, u8* dest, const u32 size, const u8 pack_size) {
      ...
      for(u32 src_i = 0; src_i < size; ++src_i) {
        if(rest >= pack_size) {  // There is space for another value.
        ...
        } else {  // There is not enough space for another value, thus wrap the value.
          ++dest_i;
          ...
          layout.data[dest_i - 1] |= ((value & left_mask) >> (pack_size - left_bits));
    ...
    //---------------------------------------------------------------------------
    template <typename T>
    static void decompress(INTEGER* dest, const u8* src, const u32 size, const u8 pack_size) {
      ...
      for(u32 dest_i = 0; dest_i < size; ++dest_i) {
        if(rest >= pack_size) {  // There is another value in current T.
          rest -= pack_size;
          dest[dest_i] = static_cast<INTEGER>((layout.data[src_i] >> rest) & pack_mask);
          ...
          T right_mask = ((1ULL << (right_bits + 1)) - 1) << (t_size - right_bits);
          ...
          T right_part = (layout.data[src_i] & right_mask) >> (t_size - right_bits);
